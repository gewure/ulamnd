...
const v = JSON.parse(readFileSync("thesis/variance.json", "utf8"));
const rPath = existsSync("thesis/results-full.json") ? "thesis/results-full.json" : "thesis/results.json";
...
writeFileSync("paper/data/summary-rows.tex", summaryRows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/cell-rows.tex", cellRows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/t2-rows.tex", t2rows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/t3-rows.tex", t3rows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/spiral-rows.tex", spiralRows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/t4-rows.tex", t4rows.join("\n") + "\n\\bottomrule\n");
...
writeFileSync("paper/data/t7-rows.tex", t7rows.join("\n") + "\n\\bottomrule\n");
...
// ---- Ulam spiral picture data (101x101) and a 3D slice
{
  ...
  writeFileSync("paper/data/ulam101.dat", rows.join("\n") + "\n");
}

writeFileSync("paper/data/macros.tex", out.join("\n") + "\n");
...
if (existsSync("thesis/sumS.json")) {
  const S = JSON.parse(readFileSync("thesis/sumS.json", "utf8")) as { H: number; P: number; CHECK: number[]; recs: { family: string; d: number; poly: string; C: number; full: number[]; diag: number[]; slope: number; slopePrev: number; slopeDiag: number }[] };
  const recs = S.recs;
  ...
  writeFileSync("paper/data/sums-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
  ...
  writeFileSync("paper/data/sums-curves.dat", lines.join("\n") + "\n");
  writeFileSync("paper/data/sums-named.tex", named.map((r) => `$${r.poly.replace(/\^(\d)/g, "^{$1}")}$ ($C=${f(r.C)}$)`).join("; ") + "\n");
...
if (existsSync("thesis/offdiag.json")) {
  const O = JSON.parse(readFileSync("thesis/offdiag.json", "utf8")) as { CHECK: number[]; DMAX: number; P_S: number; recs: { poly: string; C: number; sums: number[]; diag: number[]; off: number[]; ref: number[] }[] };
  ...
  writeFileSync("paper/data/off-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
  writeFileSync("paper/data/macros.tex", out.join("\n") + "\n");
...
if (existsSync("thesis/riesz.json")) {
  const Rz = JSON.parse(readFileSync("thesis/riesz.json", "utf8")) as { X: number; C: number; Lzeros: number[]; Zzeros: number[]; resolution: number; matches2: { om: number; power: number; src: string; v: number; dist: number }[]; peaks1: { om: number; p: number }[] };
...
if (existsSync("thesis/rieszd.json")) {
  const D = JSON.parse(readFileSync("thesis/rieszd.json", "utf8")) as { X: number; resolution: number; an2: { meanZL: number; meanZZ: number; meanZR: number; sdZR: number; seL: number; seZ: number; nL: number; nZ: number }; matches: { om: number; power: number; src: string; v: number; dist: number }[] };
  ...
  writeFileSync("paper/data/rieszd-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
  mac("rdTopWithin", String(D.matches.slice(0, 5).filter((m) => m.dist <= 0.15).length));
}
if (existsSync("thesis/riesz-fit.json")) {
  const Fj = JSON.parse(readFileSync("thesis/riesz-fit.json", "utf8")) as { nL: number; nZ: number; r2pred: number; r2L: number; r2Z: number; controls: { delta: number; r2: number }[]; randomMean: number; randomP95: number; randomMax: number; pValue: number };
...
if (existsSync("thesis/galois.json")) {
  const G = JSON.parse(readFileSync("thesis/galois.json", "utf8")) as { group: string; r: number; C: number; slopeC: number }[];
  const sdv = (xs: number[]) => Math.sqrt(mean(xs.map((x) => (x - mean(xs)) ** 2)));
...
if (existsSync("thesis/exact.json")) {
  const X = JSON.parse(readFileSync("thesis/exact.json", "utf8")) as {
    H: number;
...
const Dname = (D: string) =>
  D.split(",")
    ...
    .reverse()
    .join("+");
if (existsSync("thesis/ff.json")) {
  const FF = JSON.parse(readFileSync("thesis/ff.json", "utf8")) as { q: number; D: string; C: number; P1: number; rows: { N: number; count: number; lhs: number; diag: number; diagTail: number; constTerm: number; off: number }[] }[];
  const rows: string[] = [];
  let offMax = 0,
    swing = 0;
  for (const r of FF) {
  ...
  rows.pop();
  writeFileSync("paper/data/ff-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
...
if (existsSync("thesis/ff-tail.json")) {
  const FT = JSON.parse(readFileSync("thesis/ff-tail.json", "utf8")) as { q: number; D: string; rows: { N: number; M: number; nD: number; Tact: number; TactBrute: number; Texp: number; offDirect: number; offLHS: number }[] }[];
  const rows: string[] = [];
  let maxDev = 0,
    maxBrute = 0,
    nBrute = 0;
  ...
  rows.pop();
  writeFileSync("paper/data/fftail-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
