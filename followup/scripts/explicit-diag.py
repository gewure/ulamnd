...
What this script does
  ...
  3. reads followup/data/riesz-raw.dat, subtracts the exact smooth terms (only the x^{m+1} coefficient
...
Writes followup/data/explicit-diag.json and followup/data/explicit-diag-{m}.dat.
"""
...
raw = json.load(open("followup/data/riesz-raw.json"))
Df1 = Df(1)
...
# ---------------------------------------------------------------- data comparison
rows = np.loadtxt("followup/data/riesz-raw.dat", skiprows=1)
u, x = rows[:, 0], rows[:, 1]
...
import pickle, os
TERMS_FILE = f"followup/data/explicit-diag-terms-K{K}-P{P}-G{int(GMAX)}{'-k4' if 4 in FAMILIES else ''}.pkl"
...
for m in MS:
    ...
    np.savetxt(f"followup/data/explicit-diag-{m}.dat", np.column_stack([u, x, resid, pred, residB]), header="u x resid pred residB", comments="", fmt="%.10g")

json.dump(results, open("followup/data/explicit-diag.json", "w"), indent=1)
print(f"done ({el()})")
