...
mkdirSync("followup/data", { recursive: true });
writeFileSync(
  "followup/data/riesz-raw.dat",
  "u x R1 R2 R3\n" + us.map((u, i) => `${u.toPrecision(17)} ${xs[i].toPrecision(17)} ${R1[i].toPrecision(17)} ${R2[i].toPrecision(17)} ${R3[i].toPrecision(17)}`).join("\n") + "\n",
);
writeFileSync("followup/data/riesz-raw.json", JSON.stringify({ X, A0, logA0, sumA, sumAd, uMin, uMax, M }, null, 1));
log(`done (${el()})`);
