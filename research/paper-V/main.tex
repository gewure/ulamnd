...
its proof (\S7.2--\S7.4 there, together with \S7.3 of~\cite{GrimmeltMerikoski2024divisor}, which gives $\delta_1=\min\{\delta,\delta\,AD\}$) one may take $\delta^{-10}$ whenever
$AD\ge1$, which holds in our application, and $\delta^{-20}$ unconditionally. We do not optimise this: below we fix a constant $C=24$, and the parameters are chosen \emph{after}
$C$, so that only the fact that the exponent is absolute is used. With $c>0$ given, $\eta'=c/(200C)$, $\delta=\eta'/2$, $\eta=c/100$ and $\delta_3=\delta$, the constraints
$\delta<\eta$, $\delta<2\eta'$ and $C\eta'+2\eta+O(\delta)<\tfrac32c(1-2\theta)$ all hold, and $\delta_S(c)=2\delta/3=c/(600C)$.
\end{remark}
