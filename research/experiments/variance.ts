...
writeFileSync("research/experiments/variance.json", JSON.stringify({ full: FULL, nPoly: N_POLY, cells: all, fit }, null, 1));
// pgfplots data files
...
for (const fam of ["spiral", "random", "linear"]) {
  ...
  writeFileSync(`research/paper-I/data/variance-${fam}.dat`, header + rows.join("\n") + "\n");
}
for (const d of [2, 3, 4, 5, 6]) {
  ...
  writeFileSync(`research/paper-I/data/variance-d${d}.dat`, header + rows.join("\n") + "\n");
}
writeFileSync(
  "research/paper-I/data/variance-fit.tex",
