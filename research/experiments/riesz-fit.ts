...
const rows = readFileSync("research/paper-I/data/rieszd-residual.dat", "utf8").trim().split("\n").slice(1).map((l) => l.split(" ").map(Number));
const u = rows.map((r) => r[0]), y = rows.map((r) => r[2]);
const R = JSON.parse(readFileSync("research/experiments/rieszd.json", "utf8")) as { Lzeros: number[]; Zzeros: number[] };
...
writeFileSync("research/experiments/riesz-fit.json", JSON.stringify(out, null, 1));
