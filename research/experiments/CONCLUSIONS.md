...
`research/experiments/variance.ts` measures the variance deficit systematically (three polynomial families, degrees
...
`research/experiments/sumS.ts` computes Σ_f(H) = Σ_{h≤H}(S_f(h) − C²) to H = 10⁵ (Euler products to p ≤ 60000) for
...
Done in `research/experiments/exact.ts`: 25 quadratics with 0.39 ≤ C ≤ 6.64, H ≤ 10⁶, no Euler truncation.
...
(`research/experiments/ff.ts`). Companion note `paper/routes.tex` scaffolds the two routes to a proof.
...
Both were verified by two independent exact computations (`research/experiments/ff.ts`, `research/experiments/ff-tail.ts`).


## Correction (9 September 2026, evening)

The exact C-vs-C² test (`research/experiments/exact.ts`) truncated ∏_p P_p at p ≤ 2·10⁶ without a tail; the truncation
