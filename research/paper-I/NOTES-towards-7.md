...
- **Exact C-versus-C² test.** Theorem 5 makes S_f(h) computable with no Euler-product truncation.
  `research/experiments/exact.ts` implements it (sieve of h and Q(h)=a²h²−D with the roots ±√D/a mod p; cofactor is 1
...
- **Function field added as §3.6**: Theorem 8 (off-diagonal vanishes identically for all moduli of
  ...
  q = 3, 5, 7 (`research/experiments/ff.ts`): diagonal increments → 1/C(f), tail bounded (swing ≤ 0.88 over N ≥ 2).
...
- **Function-field identity corrected and strengthened.** The earlier statement of Theorem 8 (with the
  ...
  (`research/experiments/ff.ts` from the S_f(h); `research/experiments/ff-tail.ts` from root pairs via CRT, plus brute force for small
