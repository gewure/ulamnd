...
loss. The third is repaired below (Lemma~\ref{lem:trunc}); the others are not. Consequently Theorem~\ref{thm:main}
is proved as an exact spectral expansion with identified main terms, and the bound $O(Y^{m+1/2-\delta})$ on its
remainder is \emph{not established}; Remark~\ref{rem:gaps} lists what is missing. Record: \texttt{research/ERRATA.md},
items~13 and~16.}}
...
\begin{remark}[what is proved, what is not; added 13 September 2026]\label{rem:gaps}
Proved: the exact identity $S(t)=\sum_e\mu(e)S_e(t)$ with each $S_e$ a sum over the finitely many
$\Gamma_0(e)$-orbits of $\mathcal H_e$ (Steps~1--3, Lemma~\ref{lem:subfamily}); $P_e[\Phi^\flat_t]\in L^2$ and hence
the spectral expansion in $L^2$ (Lemma~\ref{lem:trunc}, with the coset count added on 13 September); the vanishing of
the constant terms (Lemma~\ref{lem:trunc}(iii)); the exact value of the untruncated coefficient integrals
(Lemma~\ref{lem:mellin}) and hence the main terms $Y^{m+1/2\pm it_j}\alpha_{j,e}$, with $\sum_j|\alpha_{j,e}|<\infty$ for
$m\ge2$ (Proposition~\ref{prop:riesz}). Not proved: that the remainder is $O(Y^{m+1/2-\delta})$, equivalently that
the three sums --- over $j$ of the non-main parts of the coefficients, over $j$ of the truncated tails, and the
continuous spectrum with a quantitative rate --- are $o(Y^{m+1/2})$. What is needed, in our present understanding:
(i)~apply the Riesz mean of order $m$ to the seed \emph{first}, so that every coefficient, not only the main term,
carries a factor decaying in $t_j$; (ii)~replace $|K_{i\tau}|\le K_0$ by a bound that keeps $e^{-\pi\tau/2}$ (uniform
bounds of the form $K_{i\tau}(x)\ll e^{-\pi\tau/2}(|x^2-\tau^2|+\tau^{2/3})^{-1/4}$ are classical) and treat the
transition region $x\asymp\tau$, which for $t_j\gg t^2$ lies in the truncated range; (iii)~make the exponent $A$ of
Lemma~\ref{lem:mellin} explicit (Stirling gives $A=1$ for the relative error $\asymp t_j(\sqrt{|D|}/t)^2$) and let (i)
absorb it; the order $m$ may have to increase. The numerical evidence of \S\ref{sec:numerics} concerns the main terms
and is unaffected. We record this rather than remove the theorem: the statement is what the data show and what the
exact expansion says; its remainder bound is an open technical problem, of the standard kind, and not the open problem
of \S\ref{sec:uniformity}.
\end{remark}
