...
cancellation with an explicit spectral expansion. We give the derivation in the form we believe it takes, resolve
its convergence obstruction, state precisely what remains, discuss the uniformity in $u$ that the Ces\`aro
...
\paragraph{Step 4: the Weyl series is a Poincar\'e series, and where the frequencies come from.} The Fourier
expansion of $\psi$ and a Mellin transform in the length reduce~\eqref{eq:orbit} to a single classical object. For
$m\in\Z$ and $\Re s>1$ let
\[
P_m(w,s)=\sum_{\gamma\in\Gamma_\infty\backslash\Gamma}(\Im\gamma w)^{s}\,\e\bigl(m\,\Re\gamma w\bigr)
\]
be Selberg's Poincar\'e series. Then, directly from the coordinates~\eqref{eq:coords},
...
\end{equation}
both sides converging absolutely for $\Re s>1$. In words: \emph{the Dirichlet series of the Weyl sums of a quadratic
congruence is Selberg's Poincar\'e series evaluated at the Heegner points of that discriminant.} This is the identity
that organises the whole paper, and it is what makes the spectrum enter. \todo{Verified numerically for $D=-4$,
$s=1.6$, $m=1$ to $10^{-16}$; the $4d$ versus $d$ convention of \S\ref{sec:walfisz} must be reconciled.}

The spectral expansion of $P_m(\cdot,s)$ is classical: unfolding gives
$\langle P_m(\cdot,s),u_j\rangle=\rho_j(m)\int_0^\infty y^{s+1/2}K_{it_j}(2\pi|m|y)\frac{dy}{y^2}$, and by
...
\end{equation}
this is meromorphic with its rightmost poles at $s=\tfrac12\pm it_j$, that is at the spectral points $s_j$ and
$1-s_j$. Evaluating at the Heegner points and summing over the orbit replaces $u_j$ by $\mathrm{Per}_D(u_j)$, so the
residue of~\eqref{eq:poincare} at $s=s_j$ is a multiple of $\rho_j(m)\,\mathrm{Per}_D(u_j)$. Mellin inversion in the
length then converts a pole at $s_j=\tfrac12+it_j$ into a term $Y^{1/2+it_j}$ of the Riesz mean, i.e.\ $\sqrt Y$
times $\cos(t_j\log Y+\varphi_j)$: exactly the observed law, with the amplitude proportional to the period, as
Table~\ref{tab:phases} confirms.

This also settles the factor-two question of Remark~\ref{rem:achieved}. The Weyl series is $\sum_dd^{-s}(\cdots)$,
and~\eqref{eq:poincare} places its poles at $s=\tfrac12\pm it_j$ in that same variable, so the frequencies are the
$t_j$. The Kloosterman-zeta route reaches the same coefficients through the modulus, in a variable normalised as
$c^{-2\sigma}$; the translation between the two normalisations is what we had wrong before the data corrected us.

There is one consequence of~\eqref{eq:poincare} that does not have to wait for the missing steps, because it needs
only the continuation of $P_m$ and not the identification of the residues.

\begin{proposition}[square-root cancellation in Hooley's sum, Riesz form]\label{prop:hooley}
Let $D$ be a discriminant, $k\ge1$, and let $T_k(X)$ be as in \S\ref{sec:hooley}. Then for every $\varepsilon>0$
\[
\frac1X\sum_{d\le X}\Bigl(1-\frac dX\Bigr)\ \sum_{b\bmod d,\ b^2\equiv D}\e\Bigl(\frac{kb}d\Bigr)\ \ll_{D,k,\varepsilon}\ X^{-1/2+\varepsilon}.
\]
\end{proposition}
\begin{proof}[Proof, modulo the classical input]
By~\eqref{eq:poincare} the Dirichlet series of the inner sums is, up to the elementary factor
$(\sqrt{|D|}/2)^{s}$ and a finite M\"obius sieve for the conditions at the primes dividing $2D$, a finite
combination of Selberg's Poincar\'e series $P_m(z_Q,s)$ over the $h(D)$ classes. In $\Re s>\tfrac12$ these continue
meromorphically with poles only at the points $s_j\in(\tfrac12,1)$ coming from eigenvalues $\lambda_j<\tfrac14$, and
with growth polynomial in $|\Im s|$ on vertical lines \cite{GoldfeldSarnak1983}. For $\Gamma=\mathrm{PSL}_2(\Z)$
there are no such eigenvalues: the smallest Laplace eigenvalue of a cusp form is
$\tfrac14+(9.5337)^2=91.14$, and the residual spectrum consists of the constants alone. So the continuation is
holomorphic throughout $\Re s>\tfrac12$, and the Riesz kernel $X^{s+1}/(s(s+1))$ decays fast enough in $|\Im s|$ to
move the Perron contour to $\Re s=\tfrac12+\varepsilon$, which gives the bound.
\end{proof}

This is the Riesz-mean form of square-root cancellation for the sums Hooley bounded by $X^{3/4+\varepsilon}$
\cite{Hooley1963} and Bykovski\u\i\ by $X^{2/3+\varepsilon}$ \cite{Bykovskii1984}. It agrees with what we see:
$T_1(X)/\sqrt X$ stays below $0.56$, $0.53$ and $0.86$ in absolute value for $D=-4,-3,-8$ over $X\le10^6$.
\todo{The proof above rests on a classical statement, the continuation of $P_m(z,s)$ to $\Re s>\tfrac12$ with poles
only at exceptional eigenvalues and polynomial growth, which we have taken from a secondary description of
\cite{GoldfeldSarnak1983} and \emph{not} verified in the original. Given that a first attempt to route this argument
through the Kloosterman zeta function produced a false prediction (Remark~\ref{rem:achieved}), the statement must be
checked against the printed theorem, in particular the normalisation of the variable and the passage from $Z_{m,n}$
to $P_m$, before this proposition is asserted. The conventions $b^2\equiv D\ (4d)$ of~\eqref{eq:gauss} and
$b^2\equiv D\ (d)$ of \S\ref{sec:walfisz} must also be reconciled.}
