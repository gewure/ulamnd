...
used here. Only $|\kappa|$, and only its dependence on $D$, enters below; the normalisations of $\rho_j$,
$\mathrm{Per}$ and $\kappa$ should be pinned to \cite{KatokSarnak1993} before any amplitude is computed
from~\eqref{eq:ks}.}
...
$u=3$ and $u=5$ (model object, $Y\le10^7$, \texttt{scripts/piece-divset.ts} with \texttt{U}) shows that the
level-one lines, which carry the signal at $u=1$ and $u=2$, are no longer detectable ($2$nd--$35$th percentile), and that
the object is dominated by a slowly varying component (single lines near $3.5$ and $5.1$ in $\log Y$, $R^2$ up to
$0.6$) whose origin we have not identified; a test for the level-$u^2$ newforms is therefore inconclusive at this
$Y$, and we record it as such (13 September 2026).} Summing $\lambda\omega(u)$ times an error $(H/u)^{1/2}u^{A}$ over $u\le H^{2/3}$ is
