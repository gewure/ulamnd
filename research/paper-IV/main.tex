...
cancellation with an explicit spectral expansion. We carry the derivation through: the root pairs are Heegner points, the
piece is a sum of one test function over a modular orbit, the convergence obstruction and the passage to $L^2$ are
resolved by a truncation and by the symmetry of the orbit, the continuous spectrum is $o(\sqrt Y)$, and the discrete
sum converges already at Ces\`aro order. One uniform Bessel estimate remains, and we say exactly what it is; discuss the uniformity in $u$ that the Ces\`aro
...
\paragraph{What now remains.} With Lemmas~\ref{lem:conventions}, \ref{lem:seed}, \ref{lem:trunc}
and~\ref{lem:Kuniform}, Remark~\ref{rem:eisenstein} and Proposition~\ref{prop:riesz}, every ingredient of
Theorem~\ref{thm:spectral} is in place: the orbit sum is legitimate, the automorphised seed lies in $L^2$, the
continuous spectrum is $o(\sqrt Y)$, the discrete sum converges already at Ces\`aro order, and the one expansion
that had to be uniform is uniform, elementarily. What is not yet done is the bookkeeping: the tail $w>W$ of each
$I_k$ must be disposed of by non-stationary phase, the errors must be summed over $k$ and then over $j$ with the
uniformity tracked, and the interchanges of summation justified. None of it requires a new idea. We state the
theorem as a target rather than a theorem because that work is not written out, and because two earlier attempts in
this paper to shorten exactly this kind of step, by appealing to a continuation theorem, both turned out to appeal
to the wrong object (Remarks~\ref{rem:achieved} and~\ref{rem:notneeded}). We would rather leave the last section
honest than claim a proof we have not written.
