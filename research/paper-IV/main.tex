...
\]
the second form again by Poisson. It has vanishing $x$-mean; for $\kappa\ge1$ (moduli $d\le Y$) it is
$\ll_N(Yy^2/|D|)\kappa^{-N}$, so the small moduli contribute $O(Y^{-N})$; for $\kappa<1$ it is $-(Yy^2/|D|)\hat w(0)$ off
the set $\{x:\ \exists n,\ (n+x)/\kappa\in\operatorname{supp}w\}$, of measure $\ll\kappa$, and $\ll y$ on it. This is the structure of
Lemma~\ref{lem:seed} with $t$ replaced by $Y$, so the coset count in the proof of Lemma~\ref{lem:trunc} shows that
$P_e[\Psi^w_Y]$ converges absolutely and uniformly on compacta, is bounded on $\Gamma_0(e)\backslash\mathbb H$, and lies in $L^2$;
its spectral expansion has no constant term. It is also continuous, $\Psi^w_Y$ being continuous.
...
contour to $\Re s=3-\varepsilon$: the residues at $s=1\pm it_j$ are, with $\Gamma(\tfrac{1-s\pm it_j}2)\sim-2/(s-1\mp it_j)$,
\[
\begin{aligned}
...
\end{aligned}
\]
and on $\Re s=3-\varepsilon$ the integrand is $\ll Y^{-7/2+\varepsilon}e^{-\pi t_j/2}(1+t_j)^{-3+\varepsilon}(1+|\Im s|)^{-N}$, so the
integral is $\ll Y^{-5/2+\varepsilon}e^{-\pi t_j/2}t_j^{-3+\varepsilon}|\tilde L_j|$ after the prefactor. Here
$|\tilde L_j|\le|\rho_j(1)|\sum_k|\lambda_j(k)|k^{-3/2}\ll e^{\pi t_j/2}t_j^{\varepsilon}$ by Hoffstein--Lockhart and Kim--Sarnak
(for oldforms, with the constants of the underlying newform), while
...
gives $\mathcal E^w(Y)=Y^{-1/2}\int c(r)Y^{ir}\,dr+O(Y^{-5/2+\varepsilon})$ with $c$ smooth and rapidly decaying (it inherits the
decay of $W_c(\tfrac32+ir)$ and the polynomial growth of $E_{\mathfrak a}(z_Q,\tfrac12+ir)$), whence $\mathcal E^w(Y)\ll_AY^{-1/2}(\log Y)^{-A}$
by integrating by parts $A$ times in $r$. The odd forms are absent by Corollary~\ref{cor:parity-e}, and the sum over
$e$ is finite.
\end{proof}
...
$\varphi=t_1\log(\pi\sqrt{|D|})+\arg\bigl(\Gamma(-it_1)W_c(\tfrac32+it_1)\bigr)+\pi\,\mathbf 1[\mathrm{Per}<0]$, and
$\mathrm{Per}\propto u_1(2i)+u_1(i)/2>0$ from the LMFDB coefficients of $u_1$. Predicted $\varphi=4.066$; observed, by
...
Since the seed is $O(Y^{-N})$ on the moduli $d\le Y$, Theorem~\ref{thm:smooth} is a statement about the window and
the far moduli of part~III, for $u=1$ and the model weights: they oscillate at the Maass parameters, at the scale
