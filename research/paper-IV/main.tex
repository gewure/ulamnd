...
\paragraph{What now remains.} With Lemmas~\ref{lem:conventions}, \ref{lem:seed} and~\ref{lem:trunc},
Remark~\ref{rem:eisenstein} and Proposition~\ref{prop:riesz}, the decomposition is complete as a structure: the
orbit sum is legitimate, the automorphised seed is in $L^2$, the continuous spectrum is $o(\sqrt Y)$, and the
discrete sum converges at Ces\`aro order. One analytic point stands between this and Theorem~\ref{thm:spectral}, and
we state it plainly rather than hide it in an estimate. The expansion~\eqref{eq:Ksmall} is an asymptotic as
$x\to0$ \emph{for fixed order}; its error term $O(x^2)$ carries a constant depending on $t_j$, and the transition
between the oscillatory and the exponentially decaying regimes of $K_{it}(x)$ occurs at $x\asymp t$. Since we apply
it at $x=\sqrt{|D|}w/t$ with $w$ ranging over the support of the sine integral, what is needed is a form
of~\eqref{eq:asymp} uniform in $t_j$, with an error saving a power of $t$ uniformly. That is a standard but genuine
piece of uniform Bessel analysis; it is the last step, and we have not carried it out.
