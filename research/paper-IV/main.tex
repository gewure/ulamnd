...
\date{Working draft of 13 September 2026 (corrected twice; see the notice)}
...
cancellation with an explicit spectral expansion. For $u=1$ we derive it on a model object, proving the exact spectral
structure and the main terms but not yet the bound on the remainder (Remark~\ref{rem:gaps}); for smooth windows in
place of the sharp cut-off the same computation gives a complete theorem (Theorem~\ref{thm:smooth}), with an
absolutely convergent spectral expansion whose parameter-free phase prediction the data confirm to $0.035$ radians.
The route: the root pairs are Heegner points, the piece is a
...
divisor sum, and this was checked numerically before the repair was written (Remark~\ref{rem:coprime}). The repair was then
read adversarially by a fresh reader (13 September, later): the repaired Steps~1--3 and Lemma~\ref{lem:subfamily} passed,
but four defects were found in the \emph{analytic} bookkeeping of the original proof, which the assessment had not
examined --- the truncated-tail bound discards a factor $e^{-\pi t_j/2}$ that the normalisation of the coefficients gives
back, so it does not sum over $j$; the non-oscillatory error terms receive no Riesz decay in $j$; the boundedness of
the Poincar\'e series was asserted from a measure statement; a paragraph claimed a saving where the arithmetic gives a
loss. The third is repaired (Lemma~\ref{lem:trunc}) and the fourth corrected. For the first two, the route of smoothing
first was carried out the same evening (Proposition~\ref{prop:exact}): it gives the exact spectral coefficients and the
main terms, and it shows that the remainder is not a technicality --- it contains Riesz-smoothed Hooley sums of the same
order as the main terms, so that the coefficients of the theorem are the residues of Proposition~\ref{prop:exact} plus a
part not yet computed (Remark~\ref{rem:gaps}). Theorem~\ref{thm:main} stands as an exact expansion with proved leading
terms; the bound on its remainder is open. What is proved in full is the smooth-window version,
Theorem~\ref{thm:smooth} (added the same night, tested before it was written). Record: \texttt{research/ERRATA.md},
items~13, 16, 17 and~19.}}
...
\[
\begin{gathered}
\mathcal P_u(Y)=\sum_{h\le Y}(Y-h)\bigl(F_u(Q_u(h))-\mathbb EF_u\bigr)-\tfrac12(\mathbb EF_u-1)Y,\\
...
$\Off^\ast_f(H)=c_{\mathrm{off}}(f)H+\sum_{u\le H^{1/2+\varepsilon}}w(u)\mathcal W_u(H/u;\log H)+O(H(\log H)^{1-c})$ (the range $H^{1/2+\varepsilon}$ since 15 September 2026; $H^{2/3+\varepsilon}$ before), and the
Ces\`aro form of Conjecture~1 of part~I is equivalent to the windows being $o(H\log H)$.

\paragraph{Results.}
\begin{itemize}
\item \emph{The spectral formula for a piece} (Theorem~\ref{thm:main}; the exact expansion and its main terms are
proved, the bound on the remainder is not --- see the correction notice and Remark~\ref{rem:gaps}): for $u=1$, fixed $f$,
Riesz means of order $m\ge2$, and the model object~\eqref{eq:model}
...
\[
Y^{-m}\mathcal P^{(m)}_u(Y)=c_{u,m}Y+Y^{1/2}\sum_{j}\bigl(\alpha_j(u,f)Y^{it_j}+\overline{\alpha_j(u,f)}Y^{-it_j}\bigr)\,\gamma_m(t_j)+O(Y^{1/2-\delta}),
\]
...
\item \emph{The smooth-window theorem} (Theorem~\ref{thm:smooth}, proved): for $w\in C_c^\infty((0,\infty))$ the
smooth-window sum $\sum_hw(h/Y)\bigl(\sigma^*_{-1}(h^2-D)-\mathbb E\sigma^*_{-1}\bigr)$ equals $Y^{-1/2}$ times an absolutely convergent
spectral sum over the even Maass cusp forms of the levels $e\mid\operatorname{rad}(2D)$, with explicit coefficients
(periods over the sub-families, Fourier coefficients, $\Gamma(-it_j)$, the Mellin transform of $w$), a continuous
part $o(Y^{-1/2})$ and an error $O(Y^{-5/2+\varepsilon})$; the phase of the first line is predicted with no free parameter
and observed to $0.035$ radians.
...
\item \emph{What the Ces\`aro conjecture needs} (\S\ref{sec:uniformity}): the sum of the pieces over $u\le H^{1/2+\varepsilon}$
converges to $c_{\mathrm{off}}H+o(H)$ provided the spectral formula holds with an error $O((H/u)^{1/2+\varepsilon}u^{A})$,
$A<\tfrac12$ (with part~III's earlier range $u\le H^{2/3+\varepsilon}$ this was $A<\tfrac14$); the level dependence of the Kuznetsov formula is therefore the whole remaining problem.
\end{itemize}

\section{The piece as a twisted Walfisz sum}\label{sec:walfisz}

Throughout, $f$ is an irreducible quadratic of discriminant $D$ and $u$ is squarefree with $\omega(u)\ge1$ prime
factors. Call a modulus $d$ \emph{admissible} (for $u$) if it is squarefree, coprime to $2Du$, and every prime
dividing it is split, $\bigl(\tfrac Dp\bigr)=1$, as in part~III. The third condition is automatic for the moduli that
actually divide a value: if $p\mid Q_u(h)=u^2h^2-D$ and $p\nmid2Du$ then $D\equiv(uh)^2\pmod p$, so $p$ is split.
It is nevertheless imposed, because the sums over admissible $d$ below (the expectation $\mathbb EF_u$, the sawtooth
series) run over all admissible moduli and not only over divisors of values, and for an inert prime $p$ the root set
$R_p$ is empty and $\lambda(p)$ is not defined. For admissible $d$ put
\[
R_d=R^{(u)}_d=\{x\bmod d:\ u^2x^2\equiv D\},\qquad \rho(d)=|R_d|=2^{\nu(d)},\qquad
...
admissible $d>1$}: including $d=1$ would add $1$ to $F_u$ and to $\mathbb EF_u$ and would spoil the symmetry used in
Lemma~\ref{lem:sawtooth}, since $R_1=\{0\}$ is not a union of pairs $\{x,-x\}$ with $x\ne-x$. The \emph{sharp piece}
is the function of the real variable $t\ge0$
\begin{equation}\label{eq:Sm}
S_u(t)=\sum_{h\le t}F_u(Q_u(h))-t\,\mathbb EF_u ,
\end{equation}
and the \emph{Riesz pieces} are its Riesz means
\begin{equation}\label{eq:Pm}
\mathcal P^{(m)}_u(Y)=\frac1{m!}\int_0^Y(Y-t)^m\,S_u(t)\,dt\qquad(m\ge0),
\end{equation}
the convention used throughout \S\S\ref{sec:formula}--\ref{sec:proof}. The expectation is subtracted at the real
argument $t$, not at $\lfloor t\rfloor$; the two agree at integers, and the sawtooth identity of
Lemma~\ref{lem:sawtooth} holds for all real $t$ only with this choice. For integer $Y$, since $\int_h^Ydt=Y-h$,
\[
\begin{aligned}
\mathcal P^{(0)}_u(Y)&=\sum_{h\le Y}(Y-h)\bigl(F_u(Q_u(h))-\mathbb EF_u\bigr)-\tfrac12\,\mathbb EF_u\,Y\\
&=\mathcal P_u(Y)-\tfrac Y2 ,
\end{aligned}
\]
with $\mathcal P_u$ the piece of the set-up paragraph, the object computed in \S\ref{sec:numerics}; the two differ by
a linear term, which every statement below carries as a nuisance term anyway.
...
\end{equation}
the series converging when the two terms $x,-x$ of each modulus are paired (it does not converge absolutely).
\end{lemma}
...
$\{(t+a)/d\}=(t+a)/d$, so by the periodicity of $\psi$ the two sawtooths sum to $-2t/d$. The alternative $a\le t$
means that some $h\le t$ satisfies $d\mid Q_u(h)$, hence $d\le u^2t^2+|D|$. So for $d>D_0(t):=u^2t^2+|D|$, which
exceeds $2t$, every pair contributes exactly $-2t/d$, and the tail of the series is
\[
\sum_{d>D_0}\frac{\lambda(d)}d\cdot\frac{\rho(d)}2\cdot\Bigl(-\frac{2t}d\Bigr)=-t\sum_{d>D_0}\frac{\lambda(d)\rho(d)}{d^2}
\ \ll\ t^{-1+\varepsilon},
\]
an absolutely convergent sum, while the range $d\le D_0$ is a finite sum. So the paired series converges,
and~\eqref{eq:sawtooth} holds, for every real $t\ge0$. \footnote{The alternative $a\le t$ does occur for $d>2t$
($D=-4$, $u=1$, $t=2$, $d=5$, $a=1$), so the finiteness of that range comes from $d\mid Q_u(a)$ and not from
$d>2t$; the first version argued the latter.}
\end{proof}
...
S_u(t)-\tfrac12F_u(Q_u(t))\,\mathbf 1_{t\in\Z}=\frac1\pi\sum_{k\ge1}\frac1k\sum_{d\ \mathrm{admissible}}\frac{\lambda(d)\rho_k(d)}{d}\,\sin\frac{2\pi kt}d ,
\end{equation}
in the following sense: for each $D_0$ the $k$-series over the finitely many moduli $d\le D_0$ converges, and the
truncated right-hand side tends to the left-hand side as $D_0\to\infty$; for $D_0\ge u^2t^2+|D|$ the omitted moduli
contribute exactly $-t\sum_{d>D_0}\lambda(d)\rho(d)d^{-2}$. Consequently, for $m\ge1$ and in the same sense,
...
\qquad \Phi_m(\theta;Y)=\frac1{m!}\int_0^Y(Y-t)^m\sin(\theta t)\,dt,
\end{equation}
with $\Phi_m(\theta;Y)\ll_m\min\bigl(\theta Y^{m+2},\ Y^{m}\theta^{-1}\bigr)$.
\end{lemma}

\begin{proof}
The Fourier series $\sum_{k\ge1}\sin(2\pi ky)/(\pi k)$ has uniformly bounded partial sums; it converges to $\psi(y)$
for $y\notin\Z$ and to $0=\psi(y)-\tfrac12$ for $y\in\Z$. Insert it into~\eqref{eq:sawtooth} for the finitely many
$d\le D_0$. For such $d$ and $x\in R_d$ the argument $(t-x)/d$ is an integer exactly when $t\in\Z$ and
$d\mid Q_u(t)$; these terms are the ones for which the series returns $\psi-\tfrac12$ instead of $\psi$, and they
account for the correction $\tfrac12\sum_{d\mid Q_u(t)}\lambda(d)/d=\tfrac12F_u(Q_u(t))$ on the left. For each $d$,
\[
\sum_{x\in R_d}\sin\frac{2\pi k(t-x)}d=\Im\Bigl[\e\Bigl(\frac{kt}d\Bigr)\overline{\rho_k(d)}\Bigr]=\rho_k(d)\,\sin\frac{2\pi kt}d,
\]
since $\rho_k(d)$ is real. The tail $d>D_0$ is the one computed in the proof of Lemma~\ref{lem:sawtooth}.
For~\eqref{eq:rieszweyl} apply $\frac1{m!}\int_0^Y(Y-t)^m\,dt$ to~\eqref{eq:weylform}: the correction term is
supported on the integers and does not affect the integral; for each fixed $d\le D_0$ the partial sums in $k$ are
uniformly bounded and converge for almost every $t$, so the integral may be taken termwise (dominated convergence);
and the integrated tail, $-\frac1{m!}\int_0^Y(Y-t)^mt\,dt\sum_{d>D_0}\lambda(d)\rho(d)d^{-2}$, tends to $0$ with
$D_0$. The bound on $\Phi_m$ follows from $|\sin\theta t|\le\theta t$ and from one integration by parts. Neither
\eqref{eq:weylform} nor~\eqref{eq:rieszweyl} is used in \S\S\ref{sec:formula}--\ref{sec:proof}, which work with the
sawtooth form directly; they are recorded because they are the form in which the modulus-side route of
\S\ref{sec:salie} sees the problem.
\end{proof}
...
of $k$ is expensive: this is the same phenomenon that forces the exponent condition in part~III,
Theorem~A ($B<(1-\theta)/(2-\theta)$; $\theta+6B<1$ in its first version), and it is the reason we do not bound the $\rho_k(d)$ one frequency at a time. The route of this paper is
...
\begin{gathered}
\mathcal P^{(m)}_u(Y)=\frac1{2\pi i}\int_{(c)}A_u(s)\,\frac{Y^{s+m+1}}{s(s+1)\cdots(s+m+1)}\,ds+\mathbb EF_u\,R_m(Y),\\
R_m(Y)=\sum_{h\le Y}\frac{(Y-h)^{m+1}}{(m+1)!}-\frac{Y^{m+2}}{(m+2)!},
\end{gathered}
\end{equation}
$R_m$ an explicit polynomial of degree $m+1$ in the integer $Y$ (Faulhaber): by~\eqref{eq:Pm},
$\mathcal P^{(m)}_u(Y)=\sum_{h\le Y}\frac{(Y-h)^{m+1}}{(m+1)!}F_u(Q_u(h))-\mathbb EF_u\frac{Y^{m+2}}{(m+2)!}$, and the
integral is Perron's formula for the Riesz sum of order $m+1$ of the coefficients of $A_u$.
...
\eqref{eq:salie}, substitute $b=\bar hy$ in~\eqref{eq:Wh}, which is a bijection of the roots of $b^2\equiv D$ onto the
roots of $y^2\equiv h^2D$ and turns $\e(2hb/p)$ into $\e(2y/p)$; hence
$\mathcal W_{2h}(D;p)=\sum_{y^2\equiv h^2D}\e(2y/p)$. Sali\'e's evaluation of the twisted Kloosterman sum,
...
Sali\'e's evaluation should be pinned to a precise statement of the normalisation. The first version substituted
$b=2\bar hy$, which turns $\e(2hb/p)$ into $\e(4y/p)$ and does not give the displayed intermediate identity.}
\end{proof}
...
singularities of $A_u$ at $s=-\tfrac12+2it_j$ and predicts an oscillation of $\mathcal P_u(Y)/\sqrt Y$ at
...
In the modulus-side route of \S\ref{sec:salie} the Weyl sums are, by Lemma~\ref{lem:salie}, Kloosterman sums of
half-integral weight, and the Kuznetsov formula of weight $\tfrac12$ would place the singularities of $W_k$ at
$s=\tfrac12\pm2it_j$ (Selberg's normalisation $c^{-2\sigma}$ together with the factor $d^{-1/2}$ of~\eqref{eq:salie}),
with residues proportional to $\overline{\rho_j(|D|)}\rho_j((k/2)^2)$, $\rho_j$ the Fourier coefficients of the
weight-$\tfrac12$ forms. That is the frequency $2t_j$ rejected in Remark~\ref{rem:achieved}; the route of
\S\ref{sec:formula} produces $t_j$. What survives from the modulus side is the arithmetic of the coefficients
$\rho_j(|D|)$: two classical inputs turn them into periods of the Maass forms of $\mathrm{SL}_2(\Z)$, and that is
what decides which forms can appear.
...
\end{equation}
with an explicit factor $\kappa$. \footnote{The identity is linear in the period, as in \cite{KatokSarnak1993}; its square, which expresses
$|\rho_j(|D|)|^2$ through $L(\tfrac12,u_j\otimes\chi_D)$, is the Waldspurger--Kohnen--Zagier form and is not what is
used here. In the level-$N$ version announced by Sugiyama \cite{Sugiyama2021}, the coefficient of the weight-$\tfrac12$
form at $-n$ is $n^{-3/4}\sum_v\Phi(z_v)/|\mathrm{Stab}(v)|$, the sum over the $\Gamma_0(N)$-classes of the forms of the
corresponding discriminant; so $\kappa$ carries the factor $|D|^{-3/4}$ and, in that normalisation, no dependence on
$t_j$. Only $|\kappa|$, and only its dependence on $D$, enters below; the constant should still be pinned to
\cite{KatokSarnak1993} before any amplitude is computed from~\eqref{eq:ks}.}
...
reversing the orientation of each; the cycle integral $\int_{\Gamma_Q\backslash S_Q}u_j\,ds$ is taken against the
unoriented hyperbolic arc length, so the orientation is immaterial and the same computation applies to
...
$x<t$; for $x>t$ it decays like $\exp\bigl(-(\sqrt{x^2-t^2}-t\arccos(t/x))\bigr)$, which is gradual just past the
turning point, about $\exp(-\tfrac{2\sqrt2}3(x-t)^{3/2}t^{-1/2})$, and exponential only well beyond it. The turning
point $x=t_j$ is
\begin{equation}\label{eq:threshold}
|D|=\Bigl(\frac{t_j}\pi\Bigr)^2,
\end{equation}
about $19.2$ for the first even form, $t_1=13.7798$. Explicitly, $|K_{it_1}(\pi\sqrt{|D|})|$ relative to its value at
$|D|=3$ is $1.0$ at $|D|=23$, $0.48$ at $27$, $0.023$ at $43$, $3\cdot10^{-4}$ at $67$ and $10^{-9}$ at $163$. Below
the turning point the periods of a fixed form are of comparable size only on average: $K_{it_1}(\pi\sqrt{|D|})$
changes sign between $|D|=3$ and $|D|=4$, so a discriminant close to a zero has an anomalously small period. The
falsifiable prediction is therefore: the first line is visible for the small discriminants, marginal around
$|D|=27$, reduced by a factor of order $50$ at $|D|=43$, and gone at $67$ and $163$; the higher lines, having larger
$t_j$, persist further. Section~\ref{sec:numerics} reports what is observed.
...
Lemma~\ref{lem:sawtooth} applies to the model object, whose moduli are all $d\ge1$ with $(d,2D)=1$: it used neither
$\lambda$ nor squarefreeness, and the modulus $d=1$, which the lemma excluded, is now kept, because the family
$\mathcal H_1$ must contain the point $z_{(1,0)}=i\sqrt{|D|}$ to be $\mathrm{SL}_2(\Z)$-invariant. Its single root
$x=0$ contributes $\psi(t)$, and $\#\{h\le t\}=\lfloor t\rfloor=t-\tfrac12+\psi(t)$; so, for real $t\ge0$,
\[
S(t):=\sum_{h\le t}\sigma^*_{-1}(h^2-D)-t\,\mathbb E\sigma^*_{-1}+\tfrac12=\sum_{(d,2D)=1}\frac1d\sum_{x\in R_d}\psi\Bigl(\frac{t-x}d\Bigr),
\qquad \mathbb E\sigma^*_{-1}=\sum_{(d,2D)=1}\frac{\rho(d)}{d^2},
\]
the series summed with $x$ and $-x$ paired. Its Riesz means $\mathcal G^{(m)}(Y)=\frac1{m!}\int_0^Y(Y-t)^mS(t)\,dt$
are the objects of Theorem~\ref{thm:main}; at integer $Y$, $\mathcal G^{(0)}(Y)=\int_0^YS(t)\,dt$ is exactly the
model object~\eqref{eq:model}, linear term included. Its terms are indexed by the pairs $(d,x)$ with $(d,2D)=1$, and by
M\"obius inversion
\[
...
root is $x=0$ at $d=1$ (the moduli are odd and coprime to $D$, so $0\notin R_d$ and $d/2\notin\Z$ for $d>1$). \footnote{The regrouping was checked in exact rational arithmetic
...
\begin{gathered}
S_e(t)=\frac1{\sqrt{|D|}}\Bigl[(\text{constant and Eisenstein terms})+\sum_{j}\langle\Phi_t,u^{(e)}_j\rangle\,\mathrm{Per}_{D,e}\bigl(u^{(e)}_j\bigr)\Bigr],\\
...
truncation does not depend on $x$ and so preserves this. For the last statement one needs a count, not only the measure statement of Lemma~\ref{lem:seed}(ii), since
$P_e$ is a discrete sum (a second reading pointed this out, 13 September 2026). Let $w$ lie in a fundamental domain
of $\Gamma$, so $\Im w\ge\sqrt3/2$, and consider the cosets $\gamma=\left(\begin{smallmatrix}\ast&\ast\\ c&d\end{smallmatrix}\right)\in\Gamma_\infty\backslash\Gamma$
with $\Im\gamma w=\Im w/|cw+d|^2\in[y,2y]$, $y\le\sqrt{|D|}$. They have $c\ll(\Im w/y)^{1/2}=:C_y$ and, for each $c$,
$\ll1+C_y$ values of $d$, so there are $\ll C_y^2\asymp1/y$ of them, and $\Re\gamma w=a/c+O(1/(c\,C_y))$ with $ad\equiv1\ (c)$.
The exceptional ones, with $\|\Re\gamma w\|\le a_t(y)=ty/\sqrt{|D|}$, have $\|a/c\|\ll ty/\sqrt{|D|}+y^{1/2}$, and for
each $c\le C_y$ the number of $a\bmod c$ with $\|a/c\|\le\eta$ is $\ll1+\eta c$; summing over $c$ gives
$\ll C_y+\eta C_y^2\ll y^{-1/2}+t/\sqrt{|D|}$ exceptional cosets in the block, each contributing $\le y$ to the sum, while
the $\ll1/y$ non-exceptional ones contribute $\ll ty^2$ each by Lemma~\ref{lem:seed}(i). The block therefore
contributes $\ll y^{1/2}+ty$, and the sum over the dyadic blocks $y\le\sqrt{|D|}$ is $\ll_D1+t$. The cosets of
$\Gamma_0(e)$ are among those of $\Gamma$, so $P_e[\Phi^\flat_t]$ is bounded (by $\ll_D1+t$) on the quotient, hence
square-integrable on the finite-volume $\Gamma_0(e)\backslash\mathbb H$. For later use we record the count itself: for $w$
in the fundamental domain, $y\le\sqrt{|D|}$ and $\eta>0$,
\begin{equation}\label{eq:cosetcount}
\#\{\gamma\in\Gamma_\infty\backslash\Gamma:\ \Im\gamma w\in[y,2y]\}\ll\frac1y,\qquad
\#\{\gamma:\ \Im\gamma w\in[y,2y],\ \|\Re\gamma w\|\le\eta\}\ll y^{-1/2}+\frac\eta y .
\end{equation}
\end{proof}
...
\text{or}\qquad m\ \ge\ 2\quad\text{(using only }\|u_j\|_\infty\ll t_j^{1/2+\varepsilon}\text{)} .
\end{gathered}
\]
...
Lemma~\ref{lem:Kuniform} is, however, \emph{not} enough for~\eqref{eq:asymp} uniformly in $j$. In $I_k$ the
argument is $x=cw$ with $c=\alpha_k/\beta_k=\sqrt{|D|}/t$, the same for every $k$; splitting the $w$-integral at
$W=\sqrt{t_j}/c$ and applying the lemma below $W$ gives an error
\[
\ll|\Gamma(it_j)|\,\frac{c^2}{t_j}\int_0^Ww^{3/2}\,dw\ \asymp\ |\Gamma(it_j)|\,c^{-1/2}\,t_j^{1/4},
\]
against a main term of size $\asymp|\Gamma(it_j)|c^{-1/2}$: a \emph{loss} of $t_j^{1/4}$, and the range $w>W$ is not
covered at all. (The first version of this paragraph claimed a saving of $t_j^{-3/4}$; the arithmetic was wrong, as a
second reading found on 13 September 2026.) The exact Mellin--Barnes evaluation of Lemma~\ref{lem:mellin} replaces
this paragraph; what it does and does not give uniformly in $j$ is discussed in Remark~\ref{rem:gaps}. Note that the
stationary point of the combined phase, where $t_j\,d(\log x)/dx$ meets $1/c$, sits at $x=\sqrt{|D|}$, independent of
$t_j$.
...
Let $\alpha,\beta>0$, $\tau\ge1$, and
\[
I(\alpha,\beta,\tau)=\int_0^\infty y^{-1/2}\sin(\beta y)K_{i\tau}(\alpha y)\,dy .
\]
...
The bound is uniform in $\tau$ but wasteful in it: $|K_{i\tau}(x)|\le K_0(x)$ discards the factor $e^{-\pi\tau/2}$ that
$K_{i\tau}$ carries for $x<\tau$, and that factor is exactly what the normalisation $|\rho_j(1)|\asymp e^{\pi t_j/2}$ of
the Fourier coefficients gives back. So the tail of the $j$-th coefficient is bounded by $e^{\pi t_j/2}e^{-2\pi kt}$,
which does not sum over $j$. Worse, for $t_j\gg kt^2/\sqrt{|D|}$ the phase $\beta y-t_j\log y$ of the integrand is
stationary at $y^\ast=t_j\sqrt{|D|}/(2\pi kt)>t$, inside the discarded range, and there the tail is of the same order
as the main term of Lemma~\ref{lem:mellin}. The truncation at height $t$ leaves the orbit sum unchanged, but the
estimate of the coefficients with $t_j\gg t^2$ is not the one the proof below uses. This was found by a second reading
on 13 September 2026; see Remark~\ref{rem:gaps}.

\begin{proposition}[the exact coefficient of the Riesz-smoothed seed; added 13 September 2026]\label{prop:exact}
For $m\ge1$ and $Y>0$ let $\Psi_Y=\frac1{m!}\int_0^Y(Y-t)^m\Phi^+_t\,dt$, so that
$\frac1{m!}\int_0^Y(Y-t)^mS_e(t)\,dt=\frac1{\sqrt{|D|}}\sum_{z\in\mathcal H_e/\Z}\Psi_Y(z)$. Then
$\Psi_Y(z)=\frac y\pi\sum_{k\ge1}k^{-1}\cos(2\pi kx)\,J_m(c_ky;Y)$ with $c_k=2\pi k/\sqrt{|D|}$ and
$J_m(\omega;Y)=\frac1{m!}\int_0^Y(Y-t)^m\sin(\omega t)\,dt$; $P_e[\Psi_Y]$ is bounded on $\Gamma_0(e)\backslash\mathbb H$ (by
$\ll_DY^{m+2}$) with vanishing $x$-mean; and for an even Maass cusp form $u_j$ of $\Gamma_0(e)$, written
$u_j(z)=2\sqrt y\sum_{n\ge1}a_j(n)K_{it_j}(2\pi ny)\cos(2\pi nx)$,
\[
\begin{aligned}
\langle P_e[\Psi_Y],u_j\rangle&=\frac{\sum_{k\ge1}a_j(k)k^{-3/2}}{\pi\sqrt{2\pi}}\cdot\frac1{2\pi i}\int_{(0)}\mathcal K_{m,t_j}(s;Y)\,ds,\\
\mathcal K_{m,\tau}(s;Y)&=\frac{\pi\,|D|^{s/2}\,Y^{m+1-s}\,2^{-s-3/2}}{2\cos(\pi s/2)\,\Gamma(m+2-s)}\,
\Gamma\Bigl(\frac{\tfrac12-s+i\tau}2\Bigr)\Gamma\Bigl(\frac{\tfrac12-s-i\tau}2\Bigr).
\end{aligned}
\]
Shifting the contour to $\Re s=\tfrac34$ crosses only the poles at $s=\tfrac12\pm it_j$, whose residues are the terms
$Y^{m+1/2\mp it_j}$ of Theorem~\ref{thm:main}; the integral on $\Re s=\tfrac34$ is
$\ll Y^{m+1/4}\,e^{-\pi t_j/2}\,t_j^{-5/4}\,\bigl|\sum_ka_j(k)k^{-3/2}\bigr|\ll Y^{m+1/4}\,t_j^{-5/4+\varepsilon}$, uniformly in $j$.
\end{proposition}
\begin{proof}
The Fourier series is the Riesz mean of the one in Step~4; the bound and the vanishing $x$-mean follow from
Lemma~\ref{lem:seed} applied for each $t\le Y$ together with the coset count in the proof of Lemma~\ref{lem:trunc}, with
$Y$ in place of $t$; hence $P_e[\Psi_Y]\in L^2$ and the coefficient is computed by unfolding as in Step~3. For each $k$
use Mellin--Parseval in $y$ with
$\int_0^\infty J_m(\omega;Y)\,\omega^{s-1}d\omega=\pi Y^{m+1-s}/(2\cos(\pi s/2)\Gamma(m+2-s))$ for $-1<\Re s<1$ (from
$\int_0^\infty\sin(\omega t)\omega^{s-1}d\omega=t^{-s}\Gamma(s)\sin(\pi s/2)$ and the Beta integral; for $m=0$ this is the
classical $-Y^{1-s}\Gamma(s-1)\sin(\pi s/2)$) and
$\int_0^\infty y^{w-1}y^{-1/2}K_{it}(\alpha y)\,dy=2^{w-5/2}\alpha^{1/2-w}\Gamma(\tfrac{w-1/2+it}2)\Gamma(\tfrac{w-1/2-it}2)$ for
$\Re w>\tfrac12$, on a line $\Re s=\sigma\in(-1,\tfrac12)$. With $\alpha_k=2\pi k$ the product
$c_k^{-s}\alpha_k^{s-1/2}=|D|^{s/2}(2\pi k)^{-1/2}$ depends on $s$ only through $|D|^{s/2}$, so the $k$-sum factors out as
$\sum_ka_j(k)k^{-3/2}$, absolutely convergent by Kim--Sarnak, which also justifies the interchanges. On the vertical
lines the integrand decays like $|\Im s|^{-m-3/4}$, the exponentials of the four $\Gamma$-type factors cancelling by
Stirling, so the shift to $\Re s=\tfrac34$ is legitimate, and between the two lines the only poles are
$s=\tfrac12\pm it_j$: $\cos(\pi s/2)$ first vanishes at $s=1$, and the next poles of the $\Gamma$-factors are at
$\tfrac52\pm it_j$. On $\Re s=\tfrac34$, with $\Im s=t$,
$|\Gamma(-\tfrac18+\tfrac i2(t_j\mp t))|\ll(1+|t_j\mp t|)^{-5/8}e^{-\pi|t_j\mp t|/4}$, $|\cos(\pi s/2)|^{-1}\ll e^{-\pi|t|/2}$ and
$|\Gamma(m+\tfrac54-it)|^{-1}\ll(1+|t|)^{-m-3/4}e^{\pi|t|/2}$; the product is $\ll e^{-\pi t_j/2}(1+|t|)^{-m-3/4}$ times
$(1+|t_j-t|)^{-5/8}(1+t_j+|t|)^{-5/8}$, whose integral over $t$ is $\ll t_j^{-5/4}$, the range $|t|\le t_j/2$ dominating.
\footnote{Validated numerically before this was written: the closed form of $J_m$ against quadrature; the Mellin--Parseval
representation against a direct quadrature with $1400$ subintervals, to $8$ digits, for $(t_j,Y)=(13.7798,50)$ and
$(5,20)$; the identity ``line at $\Re s=0$ equals line at $\Re s=\tfrac34$ minus the two residues'' to $7$ digits for
$t_j=13.78,40$ and $Y=50,5000$; and the decay of the remainder in $t_j$ and its growth $\asymp Y^m$ in $Y$.}
\end{proof}

\begin{theorem}[the spectral formula for the model object; the remainder bound is not established, Remark~\ref{rem:gaps}]\label{thm:main}
Let $D<0$ be a discriminant, $u=1$, and let $\mathcal G^{(m)}(Y)=\frac1{m!}\int_0^Y(Y-t)^mS(t)\,dt$ be the Riesz mean of order $m$ of the
sawtooth series $S$ of Step~2, whose order-$0$ mean is the model object~\eqref{eq:model} at integer $Y$. For each $e\mid\operatorname{rad}(2D)$ let $\{u^{(e)}_j\}$ be an orthonormal basis of the even
...
depend on $k$, so the relative error is uniform in $k$ and survives the $k$-sum unchanged. Collecting the main terms
gives, for the untruncated integrals,
$\sum_k(a_j(k)/\pi k)I(\alpha_k,\beta_k,t_j)=t^{-1/2}(A_j(D)t^{-it_j}+\overline{A_j(D)}t^{it_j})+O(t^{-5/2+\varepsilon}t_j^{A})$
...
against the Riesz weight turns it into $Y^{m+1/2\mp it_j}$ times $B(m+1,\tfrac12\mp it_j)\asymp t_j^{-m-1}$, which is the
main term of the statement, absolutely convergent over $j$ for $m\ge2$ by Proposition~\ref{prop:riesz}.

What this argument does \emph{not} establish is the bound on the remainder. Three things are missing, found by a
second reading on 13 September 2026 and listed in Remark~\ref{rem:gaps}: the error $O(t^{-5/2+\varepsilon}t_j^{A})$ is
not oscillatory in $t$, so the Riesz mean gives it $Y^{m-3/2}t_j^A$ with no decay in $j$, and $\sum_jt_j^A|\mathrm{Per}(u_j)|$
diverges by the Weyl law for every $A\ge0$; the truncated tail is not $O(e^{-2\pi t})$ after the sum over $j$ (see
after Lemma~\ref{lem:tail}); and at fixed $t$ the spectral sum $\sum_j\langle\Phi^\flat_t,u_j\rangle u_j(z_Q)$ is not
absolutely convergent, so the Riesz mean must be applied to the seed before the expansion, which the truncation at
the $t$-dependent height $T=t$ does not respect as written. The first version of this proof claimed all three
(``the error terms sum by Proposition~\ref{prop:riesz} with room to spare'').
\end{proof}

\begin{remark}[what is proved, what is not, and why; rewritten 13 September 2026, evening]\label{rem:gaps}
Proved: the exact identity $S(t)=\sum_e\mu(e)S_e(t)$ with each $S_e$ a sum over the finitely many
$\Gamma_0(e)$-orbits of $\mathcal H_e$ (Steps~1--3, Lemma~\ref{lem:subfamily}); the vanishing of the constant terms;
$P_e[\Phi^\flat_t],P_e[\Psi_Y]\in L^2$; the exact coefficient formula of Proposition~\ref{prop:exact}, hence the main
terms $Y^{m+1/2\pm it_j}$ with the residue coefficients, absolutely summable over $j$ for $m\ge2$.

Not proved: that the remainder is $o(Y^{m+1/2})$. The per-form remainder of Proposition~\ref{prop:exact} is
$\ll Y^{m+1/4}t_j^{-5/4+\varepsilon}$; with $|\mathrm{Per}_{D,e}(u_j)|\ll t_j^{1/2}$ and the Weyl law the sum over $j$ of these
bounds diverges (it is $\sum_jt_j^{-3/4}$ against the density $t_j\,dt_j$). The exponent $-\tfrac54$ is not improved by the
Riesz order, because it comes from the range $|\Im s|\ll t_j$ on the shifted line, where the only decay in $t_j$ is that
of the two $\Gamma$-factors; and it is not improved by shifting further right, because $1/\cos(\pi s/2)$ has a pole at
$s=1$. That pole is the obstruction, and it has a meaning. Its residue is a term $Y^m r_j$ with
$r_j\asymp t_j^{-3/2+o(1)}\mathrm{Per}_{D,e}(u_j)$, whose spectral sum converges at best conditionally; on the geometric side
it is the coefficient of $Y^m$ in $\Psi_Y(z)$ for $y\gg\sqrt{|D|}/(kY)$, namely $\tfrac{\sqrt{|D|}}2B_2(\{x\})$ with $B_2$ the
second Bernoulli polynomial --- a function of $x$ alone, the memory of the sharp start of the sum at $h=1$, which no
smoothing at the upper end removes. Summed over the points with $d\lesssim Y$ it is
$\tfrac{Y^m}2\sum_{d\lesssim Y}\sum_{x\in R_d}B_2(x/d)=\tfrac{Y^m}{2\pi^2}\sum_kk^{-2}\sum_{d\lesssim Y}\rho_k(d)$: a Riesz-smoothed
form of Hooley's Weyl sums $T_k(Y)$ of \S\ref{sec:hooley}, of size $Y^{1/2}$ conjecturally and $Y^{2/3+\varepsilon}$
provably (Bykovski\u\i), oscillating at the same $t_j$ (\S\ref{sec:hooley}). So the coefficient of $Y^{m+1/2\pm it_j}$ in
$\mathcal G^{(m)}(Y)$ is the residue of Proposition~\ref{prop:exact} \emph{plus} the contribution of these Hooley sums,
and a bound for the remainder requires the modulus-side spectral theory (Kuznetsov, Bykovski\u\i) for the latter. This
is not a technicality; the first version of this remark called it one, and the analysis above, carried out the same
day, shows why it is not. It also offers an explanation of the one prediction of this paper that the data did not
confirm, the amplitude law of \S\ref{sec:numerics}: the amplitudes we predicted were the residues alone.

What \emph{is} provable by exactly this computation is the smooth-window statement. For $w\in C_c^\infty((0,\infty))$,
Poisson summation turns $\sum_hw(h/Y)\bigl(\sigma^*_{-1}(h^2-D)-\mathbb E\sigma^*_{-1}\bigr)$ into a sum over $\mathcal H_e/\Z$ of the
seed $\tfrac{2Yy^2}{|D|}\sum_k\Re\hat w(kYy/\sqrt{|D|})\cos(2\pi kx)$, Schwartz in $Yy$; its Mellin transform in $y$ is entire
to the right and rapidly decaying, so the contour can be moved to $\Re s=\tfrac52-\varepsilon$, where the $\Gamma$-factors
give $t_j^{-3}$ and the spectral sum converges absolutely with the trivial sup-norm bound. The result has the shape
$Y^{-1/2}\sum_e\mu(e)\sum_j(\beta_{j,e}Y^{it_j}+\overline{\beta_{j,e}}Y^{-it_j})+O(Y^{-3/2+\varepsilon})$: it concerns the
increments of $S(t)$ over $[Y/2,Y]$, not $S(t)$ itself, and so does not see the Hooley part. Its shape was checked
before this paragraph was written (\texttt{scripts/piece-divset.ts}, \texttt{SMOOTH=1}): for $D=-4$, $Y\le2\cdot10^6$,
$\sqrt Y$ times the smooth-window sum is explained by the six level-one even parameters at the $100$th percentile
against random frequency sets (the odd set at the $13$th--$22$nd), and for the restricted object its largest line is
at $8.95$, the first even newform of level $2$. The proof is written out below, as Theorem~\ref{thm:smooth}.
\end{remark}

\subsection{The smooth-window theorem}\label{sec:smooth}

\begin{theorem}[the spectral formula for smooth windows]\label{thm:smooth}
Let $D<0$ be a discriminant, $w\in C_c^\infty((0,\infty))$ real, $W(\zeta)=\int_0^\infty w(u)u^{\zeta-1}du$ and
$W_c(z)=(2\pi)^{-z}\Gamma(z)\cos(\pi z/2)\,W(1-z)$. For $e\mid\operatorname{rad}(2D)$ let $\{u_j\}$ be an orthonormal basis of
the even Maass cusp forms of $\Gamma_0(e)$, $u_j(z)=2\sqrt y\sum_{n\ge1}a_j(n)K_{it_j}(2\pi ny)\cos(2\pi nx)$, and put
$\tilde L_j=\sum_{k\ge1}a_j(k)k^{-3/2}$ and $\mathrm{Per}_{D,e}(u_j)=\sum_{Q\in\Gamma_0(e)\backslash\mathcal H_e}u_j(z_Q)/|\Gamma_0(e)_{z_Q}|$
as in~\eqref{eq:spectral}. Then for $Y\ge1$ and every $\varepsilon>0$,
\[
\begin{aligned}
\sum_{h\ge1}w\Bigl(\frac hY\Bigr)\bigl(\sigma^*_{-1}(h^2-D)-\mathbb E\sigma^*_{-1}\bigr)
&=\frac{1}{|D|^{1/4}\sqrt Y}\sum_{e\mid\operatorname{rad}(2D)}\mu(e)\sum_j\mathrm{Per}_{D,e}(u_j)\,\tilde L_j\,\bigl[A_j(Y)+\overline{A_j(Y)}\bigr]\\
&\quad+\mathcal E^w(Y)+O_{w,D,\varepsilon}\bigl(Y^{-5/2+\varepsilon}\bigr),\\
A_j(Y)&=\Gamma(-it_j)\,W_c\bigl(\tfrac32+it_j\bigr)\Bigl(\frac{\pi\sqrt{|D|}}Y\Bigr)^{it_j},
\end{aligned}
\]
the sum over $j$ converging absolutely, where $\mathcal E^w$ is the contribution of the continuous spectra and
satisfies $\mathcal E^w(Y)\ll_{A}Y^{-1/2}(\log Y)^{-A}$ for every $A$.
\end{theorem}

\begin{proof}
\emph{Poisson summation.} Write $E=\mathbb E\sigma^*_{-1}=\sum_{(d,2D)=1}\rho(d)/d^2$ and
$\hat w(\xi)=\int w(u)\e(-\xi u)\,du$. Since $w$ has compact support in $(0,\infty)$,
$\sum_{h\ge1}w(h/Y)=\sum_{h\in\Z}w(h/Y)=Y\sum_k\hat w(kY)=Y\hat w(0)+O(Y^{-N})$, and for each of the finitely many $h$
in the support $\sigma^*_{-1}(h^2-D)=\sum_{(d,2D)=1}d^{-1}\mathbf 1_{d\mid h^2-D}$; so, with
$\sum_{h\equiv x\,(d)}w(h/Y)=(Y/d)\sum_{k\in\Z}\hat w(kY/d)\e(kx/d)$ and the $k=0$ terms summing to $YE\hat w(0)$,
\[
\sum_hw(h/Y)\bigl(\sigma^*_{-1}(h^2-D)-E\bigr)=\sum_{(d,2D)=1}\frac Y{d^2}\sum_{k\ne0}\hat w\Bigl(\frac{kY}d\Bigr)\rho_k(d)+O(Y^{-N}),
\]
$\rho_k(d)=\sum_{x\in R_d}\e(kx/d)$, real. (The $d$-sum is finite for the terms with a lattice point in the support and
absolutely convergent for the rest, since $\hat w$ is Schwartz.) In the coordinates~\eqref{eq:coords}, with
$\kappa=Yy/\sqrt{|D|}$ so that $Y/d^2=Yy^2/|D|$, this is $\sum_e\mu(e)\sum_{z\in\mathcal H_e/\Z}\Psi^w_Y(z)$ with the seed
\[
\Psi^w_Y(z)=\frac{Yy^2}{|D|}\sum_{k\ne0}\hat w(k\kappa)\,\e(-kx)=\frac{y}{\sqrt{|D|}}\sum_{n\in\Z}w\Bigl(\frac{(n-x)\sqrt{|D|}}{Yy}\Bigr)-\frac{Yy^2}{|D|}\hat w(0),
\]
the second form again by Poisson. (The double sum over $(d,k)$ is not absolutely convergent --- it is
$\asymp\sum_d\rho(d)/d$ in absolute value --- so the regrouping into the sub-families $\mathcal H_e$ and into orbits is
carried out on the $k$-summed second form, where each term is bounded and the $d$-sum converges as explained.) The seed
has vanishing $x$-mean. For $\kappa\ge1$ it is $\ll_N(Yy^2/|D|)\kappa^{-N}$: the moduli $d\le Y^{1-\delta}$, where
$\kappa\ge Y^{\delta}$, contribute $O(Y^{-N})$ in total, while the moduli $d\asymp Y$ do contribute. For $\kappa<1$ it is
$-(Yy^2/|D|)\hat w(0)$ off the set $\{x:\ \exists n,\ (n-x)/\kappa\in\operatorname{supp}w\}$, of measure $\ll\kappa$, and $\ll y$ on
it. So for $\Im\gamma w$ in a dyadic block $[y_0,2y_0]$ with $\kappa_0=Yy_0/\sqrt{|D|}<1$, the count~\eqref{eq:cosetcount}
with $\eta\asymp\kappa_0$ gives $\ll y_0^{-1/2}+Y/\sqrt{|D|}$ cosets on which the seed is $\ll y_0$, contributing
$\ll y_0^{1/2}+Yy_0$, and $\ll1/y_0$ cosets on which it is $\ll Yy_0^2/|D|$, contributing $\ll Yy_0$; so the block is
$\ll_D y_0^{1/2}+Yy_0$, the sum over the dyadic blocks with $Yy_0<\sqrt{|D|}$ is $\ll_D1$, and the blocks with
$Yy_0\ge\sqrt{|D|}$ are bounded by the rapid decay; hence $P_e[\Psi^w_Y]$ converges absolutely and uniformly on compacta,
is bounded on $\Gamma_0(e)\backslash\mathbb H$ (by $\ll_D1$, uniformly in $Y$), and lies in $L^2$; its spectral expansion
has no constant term. It is also continuous, $\Psi^w_Y$ being continuous.

\emph{The coefficients.} Unfolding as in Step~3 and pairing the Fourier series of $\Psi^w_Y$ with that of $u_j$,
$\int_0^1\e(-kx)\cos(2\pi nx)\,dx=\tfrac12\delta_{|k|,n}$ gives
\[
\langle P_e[\Psi^w_Y],u_j\rangle=\frac{2Y}{|D|}\sum_{k\ge1}a_j(k)\int_0^\infty y^{1/2}\,\Re\hat w(\kappa_ky)\,K_{it_j}(2\pi ky)\,dy,\qquad
\kappa_k=\frac{kY}{\sqrt{|D|}} .
\]
Since $\Re\hat w(v)=\int w(u)\cos(2\pi uv)\,du$ and $\int_0^\infty v^{z-1}\cos(2\pi uv)\,dv=(2\pi u)^{-z}\Gamma(z)\cos(\pi z/2)$
for $0<\Re z<1$, the Mellin transform of $y\mapsto y^{1/2}\Re\hat w(\kappa y)$ is $\kappa^{-s-1/2}W_c(s+\tfrac12)$ for
$-\tfrac12<\Re s<\tfrac12$; $W_c$ continues to an entire function on $\Re z>0$, and on vertical lines
$|W_c(\sigma+i\tau')|\ll(1+|\tau'|)^{\sigma-1/2}|W(1-\sigma-i\tau')|\ll_N(1+|\tau'|)^{-N}$, because $w$ is smooth. With
$\int_0^\infty y^{v-1}K_{it}(\alpha y)\,dy=2^{v-2}\alpha^{-v}\Gamma(\tfrac{v+it}2)\Gamma(\tfrac{v-it}2)$ ($\Re v>0$) and Mellin--Parseval
on $\Re s=0$,
\[
\begin{aligned}
\langle P_e[\Psi^w_Y],u_j\rangle&=\frac{2Y}{|D|}\,\tilde L_j\cdot\frac1{2\pi i}\int_{(0)}\mathcal K^w_{t_j}(s;Y)\,ds,\\
\mathcal K^w_{\tau}(s;Y)&=\Bigl(\frac Y{\sqrt{|D|}}\Bigr)^{-s-1/2}W_c\bigl(s+\tfrac12\bigr)\,2^{-1-s}(2\pi)^{s-1}\,
\Gamma\Bigl(\frac{1-s+i\tau}2\Bigr)\Gamma\Bigl(\frac{1-s-i\tau}2\Bigr),
\end{aligned}
\]
the $k$-dependence $\kappa_k^{-s-1/2}(2\pi k)^{s-1}=k^{-3/2}(Y/\sqrt{|D|})^{-s-1/2}(2\pi)^{s-1}$ having factored out into
$\tilde L_j$, which converges absolutely by Kim--Sarnak and justifies the interchanges. The integrand is entire in
$\Re s>-\tfrac12$ except for the poles of the $\Gamma$-factors at $s=1\pm it_j+2n$, and decays faster than any power of
$|\Im s|$ on vertical lines (the exponentials of the $\Gamma$-factors cancel by Stirling, $W_c$ supplies the decay). Move the
contour to $\Re s=3-\varepsilon$; the integral on $\Re s=0$ equals the integral on $\Re s=3-\varepsilon$ minus the residues
at $s=1\pm it_j$, and with $\Gamma(\tfrac{1-s\pm it_j}2)\sim-2/(s-1\mp it_j)$ minus those residues is
\[
\begin{aligned}
&\frac{2Y}{|D|}\tilde L_j\Bigl(\frac Y{\sqrt{|D|}}\Bigr)^{-3/2}\Bigl[2\cdot2^{-2-it_j}(2\pi)^{it_j}\Bigl(\frac Y{\sqrt{|D|}}\Bigr)^{-it_j}W_c\bigl(\tfrac32+it_j\bigr)\Gamma(-it_j)+\text{c.c.}\Bigr]\\
&\qquad=\frac{\tilde L_j}{|D|^{1/4}\sqrt Y}\Bigl[\Gamma(-it_j)W_c\bigl(\tfrac32+it_j\bigr)\Bigl(\frac{\pi\sqrt{|D|}}Y\Bigr)^{it_j}+\text{c.c.}\Bigr],
\end{aligned}
\]
and on $\Re s=3-\varepsilon$ the integrand is $\ll Y^{-7/2+\varepsilon}e^{-\pi t_j/2}(1+t_j)^{-3+\varepsilon}(1+|\Im s|)^{-N}$ away
from $|\Im s|\approx t_j$, and $\ll Y^{-7/2+\varepsilon}e^{-\pi t_j/2}t_j^{-3/2}(1+|\Im s|)^{-N}$ there (one $\Gamma$-factor is
then $O(1)$), which with $N$ large is again $\ll e^{-\pi t_j/2}t_j^{-3}$ after integration; so the integral is
$\ll Y^{-5/2+\varepsilon}e^{-\pi t_j/2}t_j^{-3+\varepsilon}|\tilde L_j|$ after the prefactor. Here
$|\tilde L_j|\le|\rho_j(1)|\sum_k|\lambda_j(k)|k^{-3/2}\ll e^{\pi t_j/2}t_j^{\varepsilon}$ by Hoffstein--Lockhart and Kim--Sarnak
for newforms; for the oldform space of a newform $f$ of level $e'\mid e$ we use the orthonormal basis obtained from
$\{f(dz):\ d\mid e/e'\}$ by Gram--Schmidt, whose Fourier coefficients are finite combinations of $\rho_f(1)\lambda_f(k/d)$
with coefficients bounded uniformly in $f$, because the Gram matrix of $\{f(dz)\}$ is a fixed-size matrix with entries
polynomial in the $\lambda_f(p)$, $p\mid e$, and determinant bounded below by Kim--Sarnak ($|\lambda_f(p)|\le2p^{7/64}$);
this is the treatment of oldforms in~\cite{IwaniecLuoSarnak2000}, for Maass forms verbatim. Meanwhile
$|\Gamma(-it_j)W_c(\tfrac32+it_j)|\ll t_j^{-1/2}e^{-\pi t_j/2}\cdot t_j\,|W(-\tfrac12-it_j)|\ll_Nt_j^{-N}e^{-\pi t_j/2}$.

\emph{Summation and the continuous spectrum.} With $|\mathrm{Per}_{D,e}(u_j)|\ll_{D,e}\|u_j\|_\infty\ll t_j^{1/2}$ on the compact
set containing the finitely many points, the main terms are $\ll t_j^{-N+1}$ and the remainders $\ll Y^{-5/2+\varepsilon}t_j^{-5/2+2\varepsilon}$,
both summable against the Weyl law; so the spectral series converges absolutely and uniformly on compacta, and may be
evaluated at the points $z_Q$. For each cusp $\mathfrak a$ of $\Gamma_0(e)$ the same computation with the Fourier
coefficients $\varphi_{\mathfrak a\infty}(n,\tfrac12+ir)$ of $E_{\mathfrak a}$, which are $\ll n^{\varepsilon}e^{\pi r/2}(1+r)^{\varepsilon}$,
gives $\mathcal E^w(Y)=Y^{-1/2}\int c(r)Y^{ir}\,dr+O(Y^{-5/2+\varepsilon})$. Here the two residues at $s=1\pm ir$ combine,
since $\overline{E_{\mathfrak a}(z,\tfrac12+ir)}=E_{\mathfrak a}(z,\tfrac12-ir)$, into a function of $r$ that is real-analytic
also at $r=0$, where the poles of $\Gamma(\mp ir)$ cancel between the two terms; $c$ is therefore smooth, and it and all
its $r$-derivatives are integrable, because they inherit the rapid decay of $W_c(\tfrac32+ir)$ and of its derivatives while
$E_{\mathfrak a}(z_Q,\tfrac12+ir)$ and its $r$-derivatives grow at most polynomially in $|r|$ on compacta. Hence
$\mathcal E^w(Y)\ll_AY^{-1/2}(\log Y)^{-A}$ by integrating by parts $A$ times in $r$. The odd forms are absent by Corollary~\ref{cor:parity-e}, and the sum over
$e$ is finite.
\end{proof}

\paragraph{The test, done before the theorem was written.} The theorem predicts the phase of each line with no free
parameter. For the unrestricted divisor sum at $D=-4$ (the case $e=1$ alone, $\mathcal H_1$ the two orbits of discriminant
$-16$, points $2i$ and $i$ with stabilisers of order $1$ and $2$) and $w(u)=\exp(-1/(1-(4u-3)^2))$ on $(\tfrac12,1)$, the
line $t_1=13.7798$ of $\sqrt Y\sum_hw(h/Y)(\sigma_{-1}(h^2+4)-E)$ should be $C\cos(t_1\log Y-\varphi)$ with
$\varphi=t_1\log(\pi\sqrt{|D|})+\arg\bigl(\Gamma(-it_1)W_c(\tfrac32+it_1)\bigr)+\pi\,\mathbf 1[\mathrm{Per}\,\tilde L_1<0]$, where
$\mathrm{Per}\propto u_1(2i)+u_1(i)/2>0$ and $\tilde L_1=1.866>0$ from the LMFDB coefficients of $u_1$. Predicted $\varphi=4.066$; observed, by
least squares on $Y\le2\cdot10^6$, $\varphi=4.032$: a difference of $0.035$ radians, where the opposite sign of the period
would give $3.1$ (\texttt{scripts/smooth-phase-test.py}). The amplitude was then tested as well: the $L^2$-norm of $u_1$ over the fundamental domain, computed from the same
twenty coefficients (converged to twenty digits with twelve), gives the normalisation, and the predicted amplitude
$2|D|^{-1/4}|\mathrm{Per}(u_1)\tilde L_1|\,|\Gamma(-it_1)W_c(\tfrac32+it_1)|=0.0600$ against the observed $0.0569$
(\texttt{scripts/smooth-amplitude-test.py}). Extending the data to $Y\le10^7$ and fitting the first six even lines
jointly (\texttt{scripts/smooth-two-lines.py}) sharpens this to a ratio $0.99$ and a phase difference of $0.010$ radians
for $t_1$ over the whole range ($0.998$ and $0.0001$ over its upper half), and gives for the second even form,
$t_2=17.7386$, with its own coefficients from the LMFDB, a predicted amplitude $0.0085$ and phase $5.860$ against
$0.0090$ and $5.753$ observed over the upper half of the range (the line is seven times weaker than the first and is
not resolved over the lower half). Two independent forms, every constant.

The level structure was tested on the restricted object~\eqref{eq:model} itself, which is the difference of the terms
$e=1$ and $e=2$ (\texttt{scripts/smooth-level2-test.py}). The sub-family $\mathcal H_2$ (forms of discriminant $-16$
with even leading coefficient) consists of three $\Gamma_0(2)$-orbits, represented by $[2,0,2]$, $[4,0,1]$ and $[4,4,2]$ with
stabilisers of orders $1$, $1$, $2$. (i)~The line of the first even newform of level $2$, $t=8.9229$ (LMFDB 2.0.1.4.1,
Fricke eigenvalue $+1$), occurs only in the term $e=2$, with the sign $\mu(2)=-1$; its period over the three points, its
coefficients and its norm on $\Gamma_0(2)\backslash\mathbb H$ (computed over the fundamental domain
$F\cup S(F)\cup ST(F)$, the Fricke involution giving the values near the cusp $0$) predict amplitude $0.0398$ and
phase $5.971$; observed, $0.0406$ and $6.020$. In the unrestricted object the theorem predicts the line absent, and the
fit gives $0.0009$. (ii)~The line $t_1$ receives, in the restricted object, the $e=1$ residue \emph{minus} the
contribution of the two-dimensional oldform space spanned by $u_1(z)$ and $u_1(2z)$, orthonormalised on
$\Gamma_0(2)\backslash\mathbb H$ (Gram matrix computed numerically: $\langle u_1(2z),u_1\rangle/\|u_1\|^2=0.730$); predicted
amplitude $0.0268$ against $0.0600$ for the unrestricted object, phase unchanged; observed $0.0271$ and $0.0597$, phases
$4.033$ and $4.066$. So the M\"obius resolution of the coprimality condition, the level-two newforms and the oldform
bookkeeping of the theorem are each seen in the data with their predicted sizes and signs. This is the first amplitude
prediction of this paper that the data confirm; the amplitude law for the Riesz object in \S\ref{sec:numerics} remains
unconfirmed, for the reason given in Remark~\ref{rem:gaps}.

\paragraph{The amplitude law across discriminants.} The amplitude of the line $t_1$ predicted by Theorem~\ref{thm:smooth}
for the unrestricted object is $C(D)=2|D|^{-1/4}|\mathrm{Per}_D(u_1)\tilde L_1|\,|\Gamma(-it_1)W_c(\tfrac32+it_1)|/\|u_1\|^2$
with $\mathrm{Per}_D(u_1)=\sum_Q u_1(z_Q)/|\Gamma_Q|$ over all classes of forms of discriminant $4D$, primitive or not; it
has no free parameter and depends on $D$ through the Heegner values alone. Table~\ref{tab:dsweep} compares it with the
fitted amplitude on smooth-window data to $Y=10^7$ for fifteen discriminants
(\texttt{research/explore/smooth-Dsweep.py}). For the ten discriminants with $|D|\le23$ the ratio lies within $5\%$ of
$1$ and the phase within $0.1$ rad, with class numbers up to $6$; beyond the turning point $|D|\approx19$ of the
threshold paragraph the predicted amplitude falls to the noise floor of a single line on these grids (about
$0.003$--$0.005$) and the ratios scatter accordingly, at $D=-43$ the prediction ($0.0006$) being below it, as that
paragraph anticipated. Adding four further level-one lines to the fit changes the ratios by at most $5\%$. The subtracted
mean $\mathbb E\sigma^*_{-1}$ is a floating-point Euler product over about $3\cdot10^5$ primes, with a rounding and tail error of order
$10^{-12}$; such an error adds a multiple of $Y^{3/2}$ to $\sqrt Y\,S^w(Y)$, which the linear detrending does not remove. Including
that term in the fit changes the amplitudes by at most $2.6\%$ for $|D|\le39$ and the phases by at most $0.05$ rad; the ratio at
$D=-23$ becomes $0.95$ (\texttt{research/explore/PROOFS-uniform.md}, \S6$''$).
\begin{table}[htbp]
\centering\footnotesize
\caption{The line $t_1$ across discriminants, unrestricted object, smooth window, $Y\le10^7$: predicted amplitude
$C(D)$, observed amplitude, their ratio, and the phase difference (rad). $h$ is the number of classes of forms of
discriminant $4D$ (all forms).}\label{tab:dsweep}
\begin{tabular}{@{}rrrrrr@{}}
\toprule
$D$ & $h$ & $C_{\mathrm{pred}}$ & $C_{\mathrm{obs}}$ & ratio & $\Delta\varphi$\\
\midrule
$-3$ & 2 & $0.0406$ & $0.0414$ & $1.02$ & $-0.04$\\
$-4$ & 2 & $0.0600$ & $0.0587$ & $0.98$ & $-0.01$\\
$-7$ & 2 & $0.0307$ & $0.0311$ & $1.01$ & $+0.03$\\
$-8$ & 3 & $0.0321$ & $0.0338$ & $1.05$ & $+0.10$\\
$-11$ & 4 & $0.0410$ & $0.0411$ & $1.00$ & $+0.03$\\
$-15$ & 4 & $0.0609$ & $0.0597$ & $0.98$ & $+0.05$\\
$-19$ & 4 & $0.0526$ & $0.0529$ & $1.01$ & $-0.01$\\
$-20$ & 6 & $0.0514$ & $0.0527$ & $1.03$ & $-0.02$\\
$-23$ & 6 & $0.0331$ & $0.0322$ & $0.97$ & $+0.08$\\
$-24$ & 6 & $0.0161$ & $0.0204$ & $1.27$ & $+0.13$\\
$-27$ & 6 & $0.0100$ & $0.0118$ & $1.18$ & $+0.40$\\
$-31$ & 6 & $0.0075$ & $0.0108$ & $1.45$ & $-0.35$\\
$-35$ & 8 & $0.0209$ & $0.0255$ & $1.22$ & $-0.09$\\
$-39$ & 8 & $0.0069$ & $0.0047$ & $0.68$ & $-0.26$\\
$-43$ & 4 & $0.0006$ & $0.0024$ & --- & ---\\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}[what the smooth theorem says and does not say]\label{rem:smoothmeaning}
The continuous spectrum is separated from the discrete lines only by powers of $\log Y$, not by a power of $Y$:
$\mathcal E^w(Y)\ll_AY^{-1/2}(\log Y)^{-A}$ is of the same order as the main term up to logarithms, and at $Y=10^7$
($\log Y\approx16$) that margin is modest. Any quantitative use of the theorem, including the level-$u^2$ discussion of
\S\ref{sec:uniformity}, must carry the Eisenstein term along rather than drop it.
Since the seed is $O(Y^{-N})$ on the moduli $d\le Y^{1-\delta}$ (not on all of $d\le Y$: the moduli $d\asymp Y$ contribute, a
point a second reading made precise), Theorem~\ref{thm:smooth} is a statement about the moduli $d\ge Y^{1-\delta}$, essentially
the window and the far moduli of part~III, for $u=1$ and the model weights: they oscillate at the Maass parameters, at the scale
$Y^{-1/2}$ appropriate to a sum over $h\in[Y/2,Y]$, with the explicit amplitudes. It is the increment of the sharp sum
$S(t)$ over a window, and increments do not see the Hooley part of Remark~\ref{rem:gaps}, which is why the sharp start
of $S(t)$ costs the remainder bound in Theorem~\ref{thm:main} and costs nothing here. For the pieces themselves (weight
$\lambda$, squarefree moduli) the same theorem holds at each level $m\ell^2$ after the M\"obius expansions of
\S\ref{sec:numerics}; the sum over $m$ and $\ell$ needs the levels to be uniform, a milder form of the problem of
\S\ref{sec:uniformity}.
\end{remark}
...
transform. The step that was wrong in the first version, the passage from the
restricted sum to orbits (Remark~\ref{rem:coprime}), was found by an external reader; its replacement is
Lemma~\ref{lem:subfamily} together with Step~2. A second reading then found the error bookkeeping of the remainder
insufficient (Remark~\ref{rem:gaps}); that is now the open part of the proof. The two places we had named as the
weak ones were the uniformity in
...
(\texttt{research/paper-IV/scripts/}). For each $f$ and $u$ we compute the piece $\mathcal P_u(Y)$ of the set-up paragraph ($=\mathcal P^{(0)}_u(Y)+Y/2$
by~\eqref{eq:Pm}) for all $Y\le10^7$
...
\begin{table}[htbp]
\centering\small
\caption{Fit of $\mathcal P_u(Y)/\sqrt Y$ to the even and to the odd spectral parameters of $\mathrm{SL}_2(\Z)$,
...
$\mathcal P_u$ is a Riesz mean of $\sigma_{-1}$ along $Q_u$. Repeating the regression:

\begin{table}[htbp]
\centering\small
\caption{Removing the arithmetic weight, at $Y\le10^7$, $u=1$. Fit of $\mathcal P_u/\sqrt Y$ to the six even
...
\mathcal G(Y)=\sum_{h\le Y}(Y-h)\bigl(\sigma^{*}_{-1}(h^2-D)-\mathbb E\sigma^{*}_{-1}\bigr)-\tfrac12\bigl(\mathbb E\sigma^{*}_{-1}-1\bigr)Y,\qquad
\sigma^{*}_{-1}(n)=\sum_{\substack{d\mid n\\ (d,2D)=1}}\frac1d,
\end{equation}
a Riesz mean of the divisor-type sums of Hooley and Gafurov. The linear term is the one of the set-up paragraph;
with it, $\mathcal G(Y)=\int_0^YS(t)\,dt$ at integer $Y$ for the sawtooth series $S$ of Step~2 of
\S\ref{sec:formula}, and it is what the scripts compute. The condition $(d,2D)=1$ is retained; it is what makes
the levels $e\mid\operatorname{rad}(2D)$ appear (Step~2 of \S\ref{sec:formula} and Remark~\ref{rem:coprime}).
Lemmas~\ref{lem:sawtooth} and~\ref{lem:weyl} never used $\lambda$ or squarefreeness, so they apply with the modulus
$d=1$ added (Step~2), and the Dirichlet series attached to~\eqref{eq:model} is
\[
...
windows of \emph{all} the pieces with $u\le H^{1/2+\varepsilon}$ ($H^{2/3+\varepsilon}$ before 15 September 2026), so what is wanted is Theorem~\ref{thm:main} uniformly
...
\]
Here is the difficulty, in the sharpest form we can state it. The forms that occur are those of discriminant
$4u^2D$ with $2u^2\mid b$ and $u^2\mid c$ (and, before the M\"obius inversion of Step~2 of \S\ref{sec:formula},
$(a,2Du)=1$); by the computation of Lemma~\ref{lem:subfamily} this set is stable under
$\Gamma^0(u^2)=\{\gamma:\ u^2\mid\beta\}$, a group conjugate to $\Gamma_0(u^2)$, and under no larger one of the
natural candidates, \footnote{Checked numerically for $(D,u)=(-4,2),(-8,3),(-11,3),(-4,5),(-3,2)$ against
$\Gamma^0(u^2)$, $\Gamma_0(u^2)$, $\Gamma^0(u)$ and $\Gamma(u)$: stable under the first, not under the other three. Note
$z_{[a,2u^2x,\ast]}=u\,z_{[a,2ux,\ast]}$ with $[a,2ux,\ast]$ of discriminant $4D$: piece $u$ is the level-$u$
sub-family of the discriminant-$4D$ points, dilated by $u$.} so that piece $u$ lives on the surface of level $u^2$.
Equivalently, since $x$ runs modulo $d$, the coordinate $\Re z$ runs modulo $u^2$, not modulo $1$: the seed is
invariant under $z\mapsto z+u^2$ but not under $z\mapsto z+1$, and the stabiliser of $\infty$ in $\Gamma^0(u^2)$ is
exactly the group generated by $z\mapsto z+u^2$. The relevant Poincar\'e series is therefore taken over
$\Gamma_\infty^{(u^2)}\backslash\Gamma^0(u^2)$, and in the pairing with a form of level $u^2$ at the cusp of width $u^2$ only the Fourier modes with $k\equiv0\ (u^2)$
of the level-one forms survive.
...
So the structure extends to every $u$: the argument of \S\S\ref{sec:formula}--\ref{sec:proof} applies verbatim to
$\Gamma^0(u^2)$ for each fixed $u$, and what is missing is precisely one thing: the size, in $u$, of the restricted
geometric factor, together with the uniformity in the level of the spectral expansion. \footnote{A first look at
$u=3$ and $u=5$ (model object, $Y\le10^7$, \texttt{scripts/piece-divset.ts} with \texttt{U}) showed that the
level-one lines, which carry the signal at $u=1$ and $u=2$, are no longer detectable, and that the object is dominated by
slowly varying components, single lines near $3.5$ and $5.1$ in $\log Y$, which on 13~September we could not identify.
They are identified now (14~September). Str\"omberg \cite{Stromberg2012} proved that $\Gamma_0(9)$ has no genuinely new
eigenvalues: its spectrum consists of the level-one forms, the forms of $\Gamma_0(3)$, their twists, and forms coming
from the cycloidal group $\Gamma^3$ of index three, the last with multiplicity two and in both parities. His table
begins $3.5360$ ($\Gamma^3$), $4.3881$ ($\Gamma_0(3)$, odd), $5.0987$ ($\Gamma_0(3)$, even), $5.5041$ ($\Gamma^3$),
$6.1206$ (odd), $6.6466$ ($\Gamma^3$), $6.7574$ (odd), $7.4318$ ($\Gamma^3$), $8.0389$ (even), $8.6983$ ($\Gamma^3$).
The smooth-window pieces at $u=3$ for $D=-8,-11,-20$ ($Y\le10^7$) have their peaks at $3.48$--$3.56$, $5.08$--$5.18$,
$6.59$--$6.78$ and $8.5$--$8.7$, and only traces ($R^2\le0.02$) at the odd-only eigenvalues $4.388$, $6.121$, $6.757$:
this is the even part of the $\Gamma_0(9)$ spectrum, as the level-$u^2$ picture predicts, on a level for which the LMFDB
has no data. In the joint-fit test of Table~\ref{tab:fits} the six even eigenvalues $\{3.5360,5.0987,5.5041,6.6466,7.4318,
8.6983\}$ explain $60$--$82\%$ of the variance of the sharp pieces at $D=-8,-11$ (both objects), at the $95$th--$100$th
percentile of $300$ random six-sets from $[3,9.5]$, while the six odd-only eigenvalues explain $8$--$14\%$, at the
$1$st--$5$th percentile (\texttt{research/explore/freq-set-test.ts}). For $u=5$ the strongest line is at $2.625$; Str\"omberg conjectures that $\Gamma_0(25)$ likewise has no
genuinely new forms and that the forms of the cycloidal group $\Gamma^5$ appear with multiplicity two, so we predict
$2.625$ to be the first eigenvalue of $\Gamma^5$, which we have not been able to check. A phase or amplitude test at
level $9$ needs the Fourier coefficients of the $\Gamma^3$-type forms, which we do not have. Data and peak tables:
\texttt{research/explore/STATE.md}.} Summing $\lambda\omega(u)$ times an error $(H/u)^{1/2}u^{A}$ over $u\le H^{1/2+\varepsilon}$ is
$O(H^{1-\delta})$ when $A<\tfrac12$ and $\varepsilon$ is small, so a bound $u^{1/2-\varepsilon}$ for that factor would settle the Ces\`aro
conjecture for quadratics (with part~III's first range $u\le H^{2/3}$ the threshold was $u^{1/4}$). This is the same wall part~III meets from the other side, in the guise of the exponent
condition of its Theorem~A: there, no Weyl-sum bound is uniform enough in the frequency; here, no bound is known for the
...
\paragraph{Addendum (15 September).} The size in $u$ of the restricted geometric factor has since been computed for
the lines that have a name, in a separate note~\cite{ReichardtDilation}. Piece $u$ is the level-one seed of
Theorem~\ref{thm:smooth} at length $uY$ summed over a $\Gamma_0(u^2)$-invariant family of forms of discriminant $4u^2D$; the
coefficient of a level-one line in piece $u$ is a projection onto the oldform space $\{u_j(z),u_j(uz),u_j(u^2z)\}$, and it
equals the coefficient in piece $1$ times an explicit rational function of $\lambda_j(u)$, $\sqrt u$ and $\chi_D(u)$: exactly
$u^{-3/2}$ for inert $u$, $-u^{-3/2}(1+O(\lambda_j(u)u^{-1/2}))$ for split $u$, $u^{-1/2}(1+O(\lambda_j(u)u^{-1/2}))$ for ramified $u$,
multiplicative in squarefree $u$. The line of a newform of level $u$ in piece $u$ is given by the same projection, and its
period vanishes when $u$ is inert. So the \emph{main terms} of the named lines are summable over $u$ with room to spare;
what the argument of the preceding paragraph needs, and what remains open, is a bound for the \emph{error} term uniform in
$u$, and the lines of the level-$u^2$ forms that have no name (the cycloidal-group forms of the footnote).
...
\paragraph{Related work in the level aspect.} For the full sets of Heegner points the level aspect has been treated:
Liu, Masri and Young~\cite{LiuMasriYoung2013} prove that the Heegner points of level $q$ and discriminant $D$
equidistribute as $q$ and $D$ grow, through a hybrid subconvexity bound for the $L$-functions of Hecke--Maass newforms
twisted by theta series, obtained from $L^2$-restriction norms of Maass forms on the Heegner points; Humphries and
Nordentoft~\cite{HumphriesNordentoft2025} obtain sparse equidistribution of the geometric invariants of real quadratic
fields in the level and subgroup aspects by rewriting the Weyl sums as ad\`elic period integrals attached to
Rankin--Selberg $L$-functions. The sub-families $\{2u^2\mid b,\ u^2\mid c\}$ of this section are natural objects for those
methods. Pascadi~\cite{Pascadi2026} controls the exceptional spectrum of $\Gamma_0(q)$ on average through a large sieve
inequality, which is the input one would need if exceptional eigenvalues at the levels $u^2$ had to be handled rather
than assumed absent.
