...
Heegner points of that discriminant.} \todo{Verified numerically for $D=-4$, $s=1.6$, $m=1$ to $10^{-16}$.} The proof
...
$0.012$ radians. \todo{Both standard facts were checked numerically; \eqref{eq:Ksmall} to a relative $10^{-6}$ at
...
Hypothesis~K is proved as Theorem~\ref{thm:main} in \S\ref{sec:proof}.
...
bijection. The phases agree because $b'\equiv b\pmod d$. \todo{Verified for $D\in\{-11,-8,-7,-4,-3,5,8,12\}$ and all
...
nothing, and only the finitely many $\gamma$ with $\Im\gamma w\le\sqrt{|D|}/2$ remain; on the rest of a fundamental
domain the sum is dominated by $t\sum_\gamma(\Im\gamma w)^2=tE(w,2)$, which is bounded there. So $P[\Phi^\flat_t]$ is
...
spectral sum in Hypothesis~K converges absolutely as soon as
\[
...
\todo{The bound was checked numerically at $30$ pairs with $\tau\in\{2,5,13.78,40,100,300\}$ and
...
the resulting integral in $\Im s$ converges absolutely. \todo{The main term was checked against numerical quadrature:
...
$|K_{i\tau}(x)|\le\int_0^\infty e^{-x\cosh u}du=K_0(x)$. \todo{Checked at $25$ pairs, $\tau\le120$, $x\le100$: the
