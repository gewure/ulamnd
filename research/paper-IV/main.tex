...
&=\frac{1}{|D|^{1/4}\sqrt Y}\sum_{e\mid\operatorname{rad}(2D)}\mu(e)\sum_j\mathrm{Per}_{D,e}(u_j)\,\tilde L_j\Bigl[\Gamma(-it_j)\,W_c\bigl(\tfrac32+it_j\bigr)\Bigl(\frac{\pi\sqrt{|D|}}Y\Bigr)^{it_j}+\text{c.c.}\Bigr]\\
...
would give $3.1$ (\texttt{scripts/smooth-phase-test.py}). The amplitude is not tested, because the normalisation
$\rho_j(1)$ of the $L^2$-normalised form is not available to us.
