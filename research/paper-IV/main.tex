...
\paragraph{What is missing.} Steps 1 and 3 are structurally complete, and Step 2 is an identity. Three things stand
between the above and a proof.
\begin{enumerate}
\item \emph{Convergence.} $|\Phi_t(z)|\le\tfrac12\Im z$ and $\sum_{\gamma\in\Gamma}\Im(\gamma w)$ diverges, so
$P[\Phi_t]$ is not absolutely convergent and the unfolding of Step 3 is formal as written. The divergence is exactly
that of the Eisenstein series at $s=1$, and it must be removed either by subtracting the mean, which is what
$\mathbb E\sigma^{*}_{-1}$ does on the arithmetic side, or by regularising in an auxiliary variable and continuing.
This is the technical heart of the matter and it is what Bykovski\u\i\ \cite{Bykovskii1984} carries out for the
closely related sums $\sum_{|n|\le P}\sigma_{-s}(n^2+D)$.
...
$\delta$, and it is where the standard bounds for $\sum_j|\mathrm{Per}_D(u_j)|^2$ and the Weyl law enter.
\end{enumerate}
