 ...
 *    LEVEL=2|3 npx tsx research/paper-IV/scripts/piece-level2.ts <tag> [nExtra=8]
...
const LEVEL = Number(process.env.LEVEL ?? 2); // newform parameters from LMFDB (fetched 13 Sep 2026), levels 2 and 3
...
const lvl2even = lists[LEVEL].even, lvl2odd = lists[LEVEL].odd;
...
const rand: number[] = []; for (let k = 0; k < 400; k++) rand.push(fit(Array.from({ length: nExtra }, () => 4 + 21 * rnd()), resid).r2);
