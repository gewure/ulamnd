 ...
 *   D=-4 Y=10000000 npx tsx research/paper-IV/scripts/piece-divset.ts
...
const D = Number(process.env.D ?? -4), Y = Number(process.env.Y ?? 10_000_000);
...
const Pmax = Y + Math.ceil(Math.sqrt(Math.abs(D))) + 2;
...
for (let h = 1; h <= Y; h++) Qv[h] = Math.abs(h * h - D);
let EFc = 1, EFa = 1;
...
console.log(`D=${D} Y=${Y}: special ${special.join(",")}, E F_coprime = ${EFc.toFixed(12)}, E F_all = ${EFa.toFixed(12)}, sigma_{-1}(|D|) = ${sig.toFixed(6)}, ${big} large cofactors (${el()})`);
