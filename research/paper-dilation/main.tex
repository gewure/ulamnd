...
$\sum_{u\text{ odd prime}}|w(u)|\,(H/u)^{1/2}\,|r_j(u;D)|\ll_D H^{1/2}$: the sum over $u\nmid D$ converges absolutely like
$\sum_uu^{-2}$, and the finitely many $u\parallel D$ contribute $\ll_DH^{1/2}$. (The weights of part~III are
...
$-0.2323=0.7303\times(-0.3181)$, $-0.0955=0.3001\times(-0.3181)$, \dots). We conjecture $r_j(u;D)=\prod_{p\mid u}r_j(p;D)$ for squarefree $u$. The evidence is the computation just described, which uses
only Theorem~\ref{thm:pieceu} and the direct evaluation of periods and Gram matrices (valid for $u=2$ as well); a proof would
show that the period vector and the Gram matrix are tensor products over the primes dividing $u$, which for odd primes
Lemmas~\ref{lem:mult}--\ref{lem:gram} make plausible but which we have not written out, and which at $2$ needs the
$2$-adic constants. The data at $u=6$ are noisy (the level-$36$ spectrum is dense):
...
\item The tensor-product argument for composite $u$.
