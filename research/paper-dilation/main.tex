...
\[
\begin{aligned}
r_j(u;D)&=u^{-3/2}&&\text{($u$ inert in $\Q(\sqrt D)$; exactly, for every $\lambda_j(u)$),}\\
r_j(u;D)&=-u^{-3/2}\bigl(1-2\lambda_j(u)u^{-1/2}+O(u^{-1})\bigr)&&\text{($u$ split),}\\
r_j(u;D)&=u^{-1/2}\bigl(1-\lambda_j(u)u^{-1/2}+(\lambda_j(u)^2+1)u^{-1}+O(u^{-3/2})\bigr)&&\text{($u\parallel D$),}
\end{aligned}
\]
the implied constants absolute. The Hecke eigenvalue enters only the corrections; the decay is set by the splitting type.
\end{theorem}
