...
\end{theorem}
\status{Lemmas~\ref{lem:mult} and~\ref{lem:period} are proved below for $u\nmid D$ and for $u\parallel D$ with $u$ not
dividing the conductor of $D$; Lemma~\ref{lem:gram} is a standard unfolding whose entries are fixed numerically to four digits
and whose write-up is pending. The values of~\eqref{eq:r} agree with the direct spectral computation to four digits and with
the data to the noise (Table~\ref{tab:ratios}).}
...
$\alpha\gamma^{-1}$ of determinant $u$. The map $\gamma\mapsto\Gamma\alpha\gamma^{-1}$ is a bijection of $\Gamma/\Gamma_0(u)$ onto
the $u+1$ cosets $\Gamma\backslash\Delta_u$ of the Hecke operator, and $\gamma\Gamma_0(u)$ is determined by $\ell_1=\langle\gamma e_1\rangle$
alone. Hence $u_j(\alpha\gamma^{-1}z_0)=:u_j(h_{\ell_1}(z_0))$ depends only on $\ell_1$, the points $h_\ell(z_0)$, $\ell\in\mathbb P^1(\mathbb F_u)$,
...
\[
\Per_{\mathcal W_u}(u_j)=\sum_C\frac1{|S_C|}\sum_{\ell_1}N(\ell_1)\,u_j(h_{\ell_1}(z_C)),\qquad N(\ell_1)=\#\{\ell_2\ne\ell_1:\ \ell_1\perp\ell_2\}.
\]
If $u\nmid D$ then $N(\ell_1)=1$ for non-isotropic and $0$ for isotropic $\ell_1$, so the inner sum is
...
If $u\parallel D$ with $u$ prime to the conductor, $N(r)=u$ and $N(\ell_1)=1$ for $\ell_1\ne r$, so the inner sum is
...
determined by $\ell_2$; the counts $\#\{\ell_1\ne\ell_2:\ell_1\perp\ell_2\}$ equal the $N(\ell_1)$ by the symmetry of $B$, which gives
$\Per_{\mathcal W_u}(u_j(u^2\cdot))=\Per_{\mathcal W_u}(u_j)$.
\end{proof}
...
\begin{enumerate}
\item Lemma~\ref{lem:gram} (a standard unfolding) to be written out; the case $u$ dividing the conductor of $D$; the
vanishing of the newform period for inert $u$ (the same isotropic-line argument should give it).
