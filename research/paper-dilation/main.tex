...
\]
where $u\parallel D$ means $u\mid D$, $u^2\nmid D$ (the case $u^2\mid D$ is excluded: there the multiplicity of Lemma~\ref{lem:mult}
is not class-independent, as at $u=2$).
Moreover
\[
\begin{aligned}
...
\[
\sqrt u\,|D|^{-1/4}\Bigl[\sum_{k=1}^2\Per_{\mathcal W_u}(v_k)\tilde L(v_k)\Bigr]\bigl[A_v(uY)+\overline{A_v(uY)}\bigr],\qquad
