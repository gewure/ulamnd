...
40. **Paper III: Theorem typeII extended from u > H^{2/3+ε} to u > H^{1/2+ε}; Theorem A now under B < (1−θ)/(2−θ) instead of
    ...
    by fresh model instances; the paper text was read by a fresh model instance — sound, 10 minor fixes applied and verified; no human check).** Not a correction of an error: the range 2/3 came from
    ...
    u with the moduli inside, blocks H^{1/2+ε} ≤ U < H^{2/3+ε}); Step 5 split at Y₀ = H^{1/2−ε} (its remainder Y₀H^{1/2+3ε′} limits Step 5
    only, not the method: below H^{1/2+ε} the part Ḡ is ≪ (log Y)^C per piece; the notes' earlier "second obstruction" is withdrawn); δ = ε/11. Consequences: Theorem A′ and Corollary smallu with u ≤ H^{1/2+ε}; Theorem E's proof with
