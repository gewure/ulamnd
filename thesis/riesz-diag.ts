...
const Rz = JSON.parse(readFileSync("thesis/riesz.json", "utf8")) as { Lzeros: number[] };
...
writeFileSync("paper/data/rieszd-spectrum.dat", "om p1 p2 p3\n" + sp1.freq.map((f, i) => `${f.toFixed(3)} ${sp1.power[i].toExponential(5)} ${sp2.power[i].toExponential(5)} ${sp3.power[i].toExponential(5)}`).join("\n") + "\n");
writeFileSync("paper/data/rieszd-detrended.dat", "om d2\n" + an2.det.map(([f, d]) => `${f.toFixed(3)} ${d.toFixed(4)}`).join("\n") + "\n");
writeFileSync("paper/data/rieszd-residual.dat", "u x n2\n" + us.filter((_, i) => i % 4 === 0).map((u, ii) => `${u.toFixed(4)} ${xs[ii * 4].toExponential(4)} ${n2[ii * 4].toExponential(5)}`).join("\n") + "\n");
writeFileSync("thesis/rieszd.json", JSON.stringify({ X, A0, resolution, an1: { ...an1, det: undefined }, an2: { ...an2, det: undefined }, an3: { ...an3, det: undefined }, matches, Lzeros, Zzeros }, null, 1));
