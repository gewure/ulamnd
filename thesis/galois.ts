...
writeFileSync("thesis/galois.json", JSON.stringify(out, null, 1));
writeFileSync("paper/data/galois-rows.tex", rows.join("\n") + "\n\\bottomrule\n");
writeFileSync("paper/data/galois-all.tex", out.map((o) => `$${o.name.replace(/\^(\d)/g, "^{$1}")}$ & $${o.group}$ & ${o.r} & ${f3(o.C)} & ${f3(o.sums[0], 2)} & ${f3(o.sums[2], 2)} & ${f3(o.slopeC)} \\\\`).join("\n") + "\n\\bottomrule\n");
