...
const ff = JSON.parse(readFileSync("thesis/ff.json", "utf8")) as { q: number; D: string; C: number; P1: number; rows: { N: number; count: number; lhs: number }[] }[];
...
writeFileSync("thesis/ff-tail.json", JSON.stringify(out, null, 1));
