...
const R = JSON.parse(readFileSync("thesis/riesz.json", "utf8")) as { Lzeros: number[]; Zzeros: number[]; resolution: number };
...
writeFileSync("thesis/riesz-analysis.json", JSON.stringify({ ...rep, zL, zZ, zC, predL, predZ, ctrl }, null, 1));
writeFileSync("paper/data/riesz-detrended.dat", "om det\n" + idx.map((i) => `${om[i].toFixed(3)} ${det.get(i)!.toFixed(4)}`).join("\n") + "\n");
