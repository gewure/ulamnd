...
writeFileSync("paper/data/riesz-spectrum.dat", "om p1 p2\n" + sp1.freq.map((f, i) => `${f.toFixed(3)} ${sp1.power[i].toExponential(5)} ${sp2.power[i].toExponential(5)}`).join("\n") + "\n");
writeFileSync("paper/data/riesz-residual.dat", "u x r1 r2\n" + us.filter((_, i) => i % 4 === 0).map((u, ii) => { const i = ii * 4; return `${u.toFixed(4)} ${xs[i].toExponential(4)} ${n1[i].toExponential(5)} ${n2[i].toExponential(5)}`; }).join("\n") + "\n");
writeFileSync("paper/data/riesz-zeros.dat", "kind gamma half\n" + Lzeros.map((g) => `L ${g.toFixed(4)} ${(g / 2).toFixed(4)}`).join("\n") + "\n" + Zzeros.map((g) => `Z ${g.toFixed(4)} ${(g / 2).toFixed(4)}`).join("\n") + "\n");
writeFileSync("thesis/riesz.json", JSON.stringify({ X, C, P1, K0, fit1: c1v, fit2: c2v, Lzeros, Zzeros, peaks1: pk1, peaks2: pk2, resolution, matches2: pk2.map((p) => ({ om: p.om, power: p.p, ...match(p.om) })) }, null, 1));
